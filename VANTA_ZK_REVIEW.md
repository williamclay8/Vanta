...
| Private Pool v2 no-witness proof artifacts | Send and actual-private-spend now emit local bb.js/UltraHonk fixture artifacts without witness or bytecode sidecars; artifact verifiers recompile the matching Noir circuit, check ACIR/verifying-key metadata, bind the single public-input hash label, and reject tampered proof bytes, public inputs, relabelled backends, and witness aliases. The operator no-witness route also binds actual-private-spend artifacts to the expected `private-spend-public-input-hash`. | `npm run private-pool-v2:send-proof-artifact-consistency-check`; `npm run private-pool-v2:actual-private-spend-proof-artifact-consistency-check`; `npm run private-pool-v2:actual-private-spend-operator-no-witness-check`; `npm run private-pool-v2:proof-backend-boundary-check` |
...
| Owner-context record-source import proof | Wallet-derived records now keep `importRequiredForCrossDevice: true` so machine-readable evidence agrees with the UI truth. A new non-secret record-source import packet proves that a second device derived from the same wallet hierarchy can match imported Shield/Send/Swap record evidence, while raw owner material is rejected and legacy local-only records remain unpromoted. | `npm run zk:owner-context-record-source-import-check`; `npm run zk:review-guards-check` |
...
`programs/vanta_private_pool_v2_spend/src/lib.rs` — and its README literally says **"no proof verification"**. The current local `process_spend` instruction takes `[1, nullifier:32, output0:32, output1:32, acceptedRoot:32, publicInputHash:32]` and anchors it after operator authority, account-binding, deterministic nullifier-marker PDA, and fixed-slot root-history checks. It checks duplicate marker reuse and account ownership/bindings, but there is still no Groth16/PLONK/Honk verifier, no verifying-key hash enforcement, and no on-chain Merkle tree that proves the accepted root came from a real shared commitment set.
...
Guard note: `npm run zk:c01-onchain-proof-boundary-check` now fails closed if C01 stops saying `partial`, if the review/ledger stop naming the missing verifier/key/tree boundary, or if verifier-like artifacts appear without replacing this negative guard with positive proof-verifier and program-owned-tree checks.
...
**Codex status, 2026-05-12 record-source import proof:** the owner-context evidence machine field now matches the UX truth: wallet-derived records remain cross-device candidates, but `importRequiredForCrossDevice` stays true until a non-secret record source is imported. `src/zk/ownerContextRecordSourceImport.ts` defines a sanitized import packet for Shield/Send/Swap record evidence, rejects raw owner context/recovery/spending/viewing material, verifies same-wallet second-device owner-context reconstruction against imported evidence hashes, and keeps legacy/random records local-only. The Shield balance recovery panel now names the "Record source import proof" boundary next to owner recovery evidence. Guard: `npm run zk:owner-context-record-source-import-check`, wired into `npm run zk:review-guards-check`. Residual caveat: this is a reviewer-visible local proof contract and truthful UI boundary, not a complete import/export UX, not legacy-record migration, not viewing-key memo discovery by itself, not pushed, and not live-verified.
...
### Partially remediated #3 — new live records use wallet-derived owner context, recovery evidence, and record-source import proof

New live Shield/Send/Swap canonical records now require an explicit wallet-derived `CanonicalNoteOwnerContext` from `useVantaShieldOwnerContext`; the bridge layer no longer generates `recoverySecret: randomHex32()`. The Shield persistence path redacts the derived recovery secret before localStorage. The current local branch also persists non-secret owner recovery evidence classification, sanitizes tainted existing evidence before reuse, propagates the evidence into lifecycle branch summaries, and exposes beta-truthful Shield/UI copy that distinguishes wallet-derived candidates from legacy local-only records. The follow-up record-source import proof keeps `importRequiredForCrossDevice: true` for wallet-derived candidates, rejects raw owner material in import packets, and proves same-wallet second-device reconstruction only when the imported record evidence matches. This closes the new-record version of the highest-impact user-facing recovery bug and prevents ambiguous owner-hint UI from being mistaken for proven cross-device recovery.

Residual risk: old browser-local records are not migrated or recovered; the user-facing cross-device export/import flow is still not complete; viewing-key backup remains a separate memo-discovery need; and nothing is pushed or live. Keep the next recovery slice focused on legacy-record migration policy and turning the local record-source proof contract into a real product import/export flow.
