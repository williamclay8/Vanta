...
**Codex status, 2026-05-23 C01 production artifact H6 binding gate:** locally tightened `ops/mainnet/private-pool-v2-c01-production-artifact-acceptance-gate.evidence.json`, `ops/mainnet/private-pool-v2-c01-sunspot-gnark-artifact-acquisition.packet.json`, and `ops/mainnet/private-pool-v2-c01-sunspot-gnark-local-artifact-inventory.evidence.json` so returned production public-witness evidence cannot promote by byte lengths alone. The acceptance gate now requires returned artifacts to match the current source ACIR hash and requires one decoded `private-spend-public-input-hash` equal to the current H6 actual-private-spend proof receipt public input and commitment, while recording that the local beta18 Sunspot/Gnark compiled ACIR and public witness are stale and comparison-only. The gate now also has an executable refs-only reviewed-bundle intake template at `ops/mainnet/private-pool-v2-c01-production-artifact-bundle.template.json`, validated by `VANTA_C01_PRODUCTION_ARTIFACT_BUNDLE_PATH=<reviewed-refs-only-json> npm run zk:c01-production-artifact-acceptance-gate-check`. Guards: `npm run zk:c01-production-artifact-acceptance-gate-check`; `npm run zk:c01-sunspot-gnark-artifact-acquisition-check`; `npm run zk:c01-sunspot-gnark-local-artifact-inventory-check`. This is production artifact intake hardening only; it is not production proof-format evidence, not production public-input binding evidence, not production verifying-key evidence, not verifier-adapter acceptance, not mutation/no-mutation evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.
