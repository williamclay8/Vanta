...
The C01 deterministic production artifact build gate, `ops/mainnet/private-pool-v2-c01-deterministic-production-artifact-build-gate.evidence.json`, records `blocked-no-deterministic-production-artifact-build-receipt` and is checked by `npm run zk:c01-deterministic-production-artifact-build-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-deterministic-production-artifact-build.template.json` can be validated with `VANTA_C01_DETERMINISTIC_PRODUCTION_ARTIFACT_BUILD_PATH=<reviewed-refs-only-json> npm run zk:c01-deterministic-production-artifact-build-check`. This is deterministic production proof/VK/public-witness build receipt intake only: it can validate source-reviewed, setup-reviewed, reproducible output refs, but it is not verifier-adapter acceptance, not mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 verifier-adapter acceptance gate, `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance-gate.evidence.json`, records `blocked-no-production-verifier-adapter-acceptance` and is checked by `npm run zk:c01-verifier-adapter-acceptance-gate-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance.template.json` can be validated with `VANTA_C01_VERIFIER_ADAPTER_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-verifier-adapter-acceptance-gate-check`. This is production verifier-adapter acceptance intake only: it can validate deterministic-build, proof/VK/public-witness, accepted adapter/program, valid mutation, and invalid/wrong-input/wrong-key no-mutation refs, but it is not tag-3 proof acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.
