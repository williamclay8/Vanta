...
**Codex status, 2026-05-20 C01 local public-witness binding observation:** locally added `ops/mainnet/private-pool-v2-c01-public-witness-binding.evidence.json` and `npm run zk:c01-public-witness-binding-check`. The packet records that the nonproduction Sunspot/Gnark 44-byte public witness decodes as a 12-byte Gnark header plus one 32-byte public input labeled `private-spend-public-input-hash`, and that value matches the local actual-private-spend Noir/bb receipt. The source tag-3 default adapter now also checks that same one-field public witness shape against `publicInputHash` before the not-wired verifier boundary. This narrows the public-witness decoding and source binding question, but it is still not production public-input binding evidence, not production proof-format evidence, not verifier-adapter acceptance, not valid-proof mutation evidence, not invalid/wrong-input/wrong-key no-mutation evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.

**Codex status, 2026-05-21 C01 verifier CPI hook hardening:** locally extended the source-only tag-3 seam so preflight now requires a dedicated read-only executable verifier-program account after the verifier-key PDA, the default adapter constructs the generated Solana verifier CPI instruction shape with no account metas and data equal to `gnarkProof || gnarkPublicWitness`, and SBF builds have an on-chain-only verifier CPI hook that passes the verifier-program account while host-side Solana syscall stubs remain fail-closed. `npm run zk:c01-verifier-adapter-seam-check` now guards the instruction shape, verifier-program preflight, public-witness binding, and host fail-closed behavior. `npm run private-pool-v2:c01-sbf-verifier-cpi-rejection-check`, now included in `npm run private-pool-v2:crucible-check`, proves the spend SBF can reach an executable verifier-program CPI and leave accounts unchanged when that verifier rejects. `npm run private-pool-v2:c01-local-unsafe-verifier-cpi-acceptance-check` proves the spend SBF can CPI into the local unsafe generated Sunspot verifier SBF from `/private/tmp`, accept the local unsafe proof/public-witness tuple, mutate nullifier/output state after verifier success, reject a tampered proof without mutation, reject a wrong public input hash without mutation, and reject a wrong executable verifier program without mutation when the verifier-key record is bound to the correct generated verifier program id. This is verifier-adapter interface and local unsafe SBF CPI hardening only. It is not production verifier adapter acceptance, not tag-3 production proof acceptance, not valid-proof mutation evidence for a production verifier, not wrong-verifying-key production no-mutation evidence, not production proof-format evidence, not production verifying-key evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.
