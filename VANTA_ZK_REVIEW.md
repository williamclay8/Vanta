...
| --- | --- | --- |
| Solana spend authority/root history/nullifier/output-record PDAs | Spend evidence writes require the initialized operator authority signer; init is one-time, pool state binds the initialized nullifier/output-index/root-history accounts, spends reject unregistered accepted roots, replay truth now uses a deterministic nullifier marker PDA, and output evidence writes use deterministic output-record PDAs instead of fixed-capacity output slots. A source-only `TAG_REGISTER_PROVENANCED_ROOT = 4` path now creates a lineage-bound program-owned root provenance record at `["vanta2root", pool_state, acceptedRoot]`; reserved tag `3` proof-carrying spend and tag `6` Unshield preflights require that record before failing closed. Legacy tag `2` roots are not backfilled into this provenance lane. Tag `3` now preflights a commit-capable account list for the root/root-record/nullifier/output/verifier-key/payer/system contract, prechecks the one-field Gnark public witness against `publicInputHash`, assembles the 368-byte proof-plus-public-witness verifier instruction-data tuple, and can CPI into a verifier SBF on Solana targets. Local harnesses now prove executable verifier-CPI rejection/no-mutation and H6 local unsafe generated-verifier CPI acceptance/mutation plus tampered-proof, wrong-public-input, and wrong-executable-program no-mutation; the optional pre-H6 wrong-verifying-key negative-control leg is skipped unless its transient verifier SBF is present and remains nonproduction evidence. The current live builder still rejects tag `3`. The local SBF ABI has been rebuilt for this source and can be cited as local bytecode evidence only; reviewed mainnet evidence remains blocked until production proof/VK/adapter, redeploy, reinit, live verification, and reviewer acceptance. | `cargo test --manifest-path programs/vanta_private_pool_v2_spend/Cargo.toml`; `npm run private-pool-v2:root-provenance-check`; `npm run private-pool-v2:contract-check`; `npm run zk:c01-onchain-proof-boundary-check`; `npm run private-pool-v2:c01-local-unsafe-verifier-cpi-acceptance-check`; `npm run private-pool-v2:sbf-abi-status`; `npm run private-pool-v2:sbf-abi-check` |
...
`programs/vanta_private_pool_v2_spend/src/lib.rs` — and its README literally says **"no proof verification"**. The current local `process_spend` instruction takes `[1, nullifier:32, output0:32, output1:32, acceptedRoot:32, publicInputHash:32]` and records it after operator authority, account-binding, deterministic nullifier-marker PDA, and fixed-slot root-history checks. The current 161-byte spend ABI carries no proof bytes; passing proof-like fields such as `onChainVerifier`, `proofBytes`, `proofArtifact`, `verifierProgramId`, or `verifyingKeyHash` into the transaction builder, relayer, service-network submitter, or operator packet is rejected or omitted rather than silently implying verifier support. A source-only `TAG_REGISTER_PROVENANCED_ROOT = 4` path now creates a lineage-bound program-owned root provenance record at `["vanta2root", pool_state, acceptedRoot]`, and reserved tag `3` plus tag `6` preflights require that record before failing closed. Legacy tag `2` roots cannot be backfilled through tag `4`; they require migration/reinitialization if they must enter the provenance lane. A source-only `TAG_REGISTER_VERIFIER_KEY = 5` path now creates or idempotently verifies the program-owned verifier-key registry PDA at `["vanta2vkey", pool_state, verifierKeyHash]` and binds it to a verifier program id, but this is not production verifying-key evidence. A reserved tag `3` proof-carrying spend ABI now exists in source as `[3, nullifier, output0, output1, acceptedRoot, publicInputHash, verifierKeyHash, gnarkProof:324, gnarkPublicWitness:44]`; it now separates preflight from default adapter rejection and a verified-commit helper in local tests, reserves a dedicated read-only executable verifier-program account, rejects a verifier program that does not match the verifier-key record before the not-wired adapter boundary, constructs the generated Solana verifier CPI instruction with no account metas and data equal to `gnarkProof || gnarkPublicWitness`, has an on-chain-only verifier CPI hook that passes the verifier-program account while host-side Solana syscall stubs remain fail-closed, and carries the writable pool/output plus payer/system accounts needed for future post-adapter mutation, but the public default path still returns custom error `14` before proof verification, account creation, nullifier/output mutation, or spend acceptance in host tests until an accepted verifier artifact is proven. The current transaction builder still rejects tag `3`. It checks duplicate marker reuse and account ownership/bindings on tag `1`, but there is still no Groth16/PLONK/Honk verifier accepted for production, no production verifying-key acceptance, no proof that the root transition is correct, and no on-chain Merkle tree that proves the accepted root came from a real shared commitment set.
...
Current C01 correction: Groth16 + Light is now the selected backend direction, not verifier readiness. The Groth16 tag-3 Solana verifier path is guarded by `docs/zk/c01-production-verifier-backend-decision.md`; it records `selectedBackend: "groth16-tag3-solana-v0"` while `ops/mainnet/private-pool-v2-c01-groth16-proof-format-candidate.evidence.json`, `ops/mainnet/private-pool-v2-c01-production-groth16-toolchain-preflight.evidence.json`, `ops/mainnet/private-pool-v2-c01-sunspot-groth16-route.evidence.json`, `ops/mainnet/private-pool-v2-c01-sunspot-groth16-dev-probe.evidence.json`, `ops/mainnet/private-pool-v2-c01-production-verifying-key-candidate.evidence.json`, `ops/mainnet/private-pool-v2-c01-sbf-live-lineage-candidate.evidence.json`, and `ops/mainnet/private-pool-v2-c01-verifier-adapter-test-candidate.evidence.json` remain blocked. The SBF/live lineage packet is guarded by `npm run zk:c01-sbf-live-lineage-candidate-check`, records `blocked-no-rebuilt-redeployed-reinitialized-live-lineage`, and keeps local SBF freshness and local unsafe verifier SBF metadata out of SBF/live lineage. The toolchain preflight status is `blocked-local-toolchain-no-groth16-scheme`: current local proof metadata is still `local-acir-bytecode-hash-not-production-vk`, and the current local `@aztec/bb.js` CLI exposes `chonk`, `avm`, and `ultra_honk` schemes but no Groth16 scheme, so it is not production proof-format evidence and not production verifying-key evidence. The first-class route packet now names `sunspot-noir-acir-gnark-groth16-solana-v0` as the candidate ACIR-to-Groth16/Solana artifact route, but that route is `blocked-sunspot-toolchain-not-installed-and-no-production-trusted-setup` with a Sunspot/Nargo compatibility blocker, `blocked-local-nargo-version-mismatch-and-sunspot-missing`: Sunspot's upstream README requires Noir/Nargo `1.0.0-beta.18`, this workspace reports `nargo 1.0.0-beta.19`, Sunspot and `GNARK_VERIFIER_BIN` are not installed locally, and a default single-operator setup is not production VK evidence without a compatible pinned/reviewed toolchain plus reviewed trusted setup ceremony or equivalent toxic-waste mitigation. A local dev probe now records `local-dev-probe-succeeded-nonproduction-unsafe-setup-and-beta18-source-shim`, proving temporary route feasibility only; it selects `gnark-solana-native-proof-and-public-witness-v0` as the candidate verifier input tuple after the generated standalone Solana verifier accepted a 324-byte proof plus 44-byte public witness. It is still blocked because the current beta19 ACIR panics Sunspot compile, the repo source required a beta18-only import shim, the spend-program tag-3 ABI now reserves that selected tuple fail-closed and rejects the legacy 256-byte proof-only shape but still lacks an accepted adapter, and Sunspot compile output reported zero public and zero secret inputs while the generated verifier records one public input.

The C01 production artifact H6 binding gate also keeps returned production public-witness evidence from passing by byte lengths alone; it cannot promote by byte lengths alone. The accepted bundle must bind the reviewed beta18 H6 production source ACIR while preserving the reference current source ACIR hash for beta19 semantic equivalence, and decode one `private-spend-public-input-hash` equal to the current H6 proof receipt public input and commitment. The older pre-H6 local beta18 compiled ACIR and public witness remain stale and comparison-only; the H6 beta18 probe witness matches the current receipt but is still local unsafe comparison evidence only.

The C01 beta18 source-migration candidate guard, `ops/mainnet/private-pool-v2-c01-beta18-source-migration-candidate.evidence.json`, records `blocked-no-reviewed-beta18-source-migration` and is checked by `npm run zk:c01-beta18-source-migration-candidate-check`. The observed beta18 shim is pre-H6 and comparison-only: it lacks the current H6 context preimage fields and Poseidon6 `computed_context_hash == context_hash` assertion, so it cannot promote without a reviewed source migration that preserves the current H6 proof receipt public input and commitment, source lineage, production artifacts, adapter acceptance, SBF/live lineage, and audit/reviewer acceptance.

The C01 beta18 H6 migration probe packet, `ops/mainnet/private-pool-v2-c01-beta18-h6-migration-probe.evidence.json`, records `local-h6-beta18-migration-probe-succeeded-nonproduction-unsafe-setup` and is checked by `npm run zk:c01-beta18-h6-migration-probe-check`. It proves a local beta18-compatible source copy can preserve the H6 context preimage fields and Poseidon6 assertion, complete Sunspot compile/setup/prove/verify, and produce a 44-byte public witness. The public witness matches the current H6 proof receipt before the generated verifier SBF and Vanta local unsafe generated-verifier CPI harness pass. This is still local unsafe route evidence only, not reviewed source migration, not production proof-format evidence, not production VK evidence, not verifier-adapter acceptance, not SBF/live lineage, and not audit/reviewer acceptance.

The C01 beta18 H6 source-migration review packet, `ops/mainnet/private-pool-v2-c01-beta18-h6-source-migration-review.evidence.json`, records `reviewable-beta18-h6-source-migration-candidate-local-only` and is checked by `npm run zk:c01-beta18-h6-source-migration-review-check`. It makes `zk/noir/vanta_private_pool_v2_actual_private_spend_entry_sunspot_beta18_h6_candidate/src/main.nr` a repo-local reviewer candidate and proves the current source already uses the beta18-compatible Poseidon import. The compatibility marker is `no-source-delta-current-uses-beta18-compatible-poseidon-import`. This is reviewer handoff evidence only, not reviewed source migration, not production source lineage, not production setup, not production proof-format evidence, not production VK evidence, not verifier-adapter acceptance, not SBF/live lineage, and not audit/reviewer acceptance.

The C01 beta18 H6 source-review acceptance gate, `ops/mainnet/private-pool-v2-c01-beta18-h6-source-review-acceptance-gate.evidence.json`, records `blocked-no-external-source-review-acceptance` and is checked by `npm run zk:c01-beta18-h6-source-review-acceptance-gate-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-beta18-h6-source-review-acceptance.template.json` can be validated with `VANTA_C01_BETA18_H6_SOURCE_REVIEW_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-beta18-h6-source-review-acceptance-gate-check`. This is external source-review intake only: it can validate reviewer acceptance of the beta18 H6 source delta, but it is not production source lineage, not production proof/VK evidence, not verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 deterministic production artifact build gate, `ops/mainnet/private-pool-v2-c01-deterministic-production-artifact-build-gate.evidence.json`, records `blocked-no-deterministic-production-artifact-build-receipt` and is checked by `npm run zk:c01-deterministic-production-artifact-build-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-deterministic-production-artifact-build.template.json` can be validated with `VANTA_C01_DETERMINISTIC_PRODUCTION_ARTIFACT_BUILD_PATH=<reviewed-refs-only-json> npm run zk:c01-deterministic-production-artifact-build-check`. This is deterministic production proof/VK/public-witness build receipt intake only: it can validate reviewed frozen source commit/source tree status refs, source-freeze review acceptance, source-reviewed, setup-reviewed, reproducible output refs plus artifact producer/reviewer identity-scope attestation, but it is not verifier-adapter acceptance, not trim-normalized distinct mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 production output-manifest preflight, `ops/mainnet/private-pool-v2-c01-production-output-manifest-preflight.evidence.json`, records `blocked-no-reviewed-production-output-manifest` and is checked by `npm run zk:c01-production-output-manifest-check`. Returned artifact directories can be checked with `VANTA_C01_PRODUCTION_OUTPUT_MANIFEST_ROOT=<returned-artifact-dir> npm run zk:c01-production-output-manifest-check`; generated refs-only manifests can be revalidated with `VANTA_C01_PRODUCTION_OUTPUT_MANIFEST_PATH=<refs-only-output-manifest-json> npm run zk:c01-production-output-manifest-check`. This can hash and shape-check proof, public-witness, VK, source-ACIR, and generated verifier-SBF refs before a deterministic build receipt cites them, but it is not production proof-format evidence, not production verifying-key evidence, not deterministic build acceptance, not verifier-adapter acceptance, and not C01 closure.

The C01 verifier-adapter acceptance gate, `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance-gate.evidence.json`, records `blocked-no-production-verifier-adapter-acceptance` and is checked by `npm run zk:c01-verifier-adapter-acceptance-gate-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance.template.json` can be validated with `VANTA_C01_VERIFIER_ADAPTER_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-verifier-adapter-acceptance-gate-check`. This is production verifier-adapter acceptance intake only: it can validate deterministic-build, proof/VK/public-witness, accepted adapter/program, valid mutation, invalid/wrong-input/wrong-key/wrong-program no-mutation refs, and reviewer identity-scope attestation, but the five mutation/no-mutation refs must be distinct. It is not tag-3 proof acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 SBF/live lineage acceptance gate, `ops/mainnet/private-pool-v2-c01-sbf-live-lineage-acceptance-gate.evidence.json`, records `blocked-no-sbf-live-lineage-acceptance` and is checked by `npm run zk:c01-sbf-live-lineage-acceptance-gate-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-sbf-live-lineage-acceptance.template.json` can be validated with `VANTA_C01_SBF_LIVE_LINEAGE_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-sbf-live-lineage-acceptance-gate-check`. This is SBF/live lineage intake only: it can validate reviewed production artifact and adapter refs, rebuilt spend/verifier SBF hashes, deployed program ids, verifier program upgrade-authority status ref, verifier-key record binding, deployment/migration/registration refs, and a live proof-enforced tag-3 or reviewer-accepted dry-run receipt, but it is not SBF/live lineage by itself, not audit/reviewer acceptance, and not C01 closure.

The C01 audit/reviewer acceptance gate, `ops/mainnet/private-pool-v2-c01-audit-reviewer-acceptance-gate.evidence.json`, records `operator-skipped-control` and is checked by `npm run zk:c01-audit-reviewer-acceptance-gate-check`. The refs-only template `ops/mainnet/private-pool-v2-c01-audit-reviewer-acceptance.template.json` can be validated with `VANTA_C01_AUDIT_REVIEWER_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-audit-reviewer-acceptance-gate-check`. This is final C01 audit/reviewer intake only: it can validate reviewed production artifact bundle, adapter acceptance, valid mutation, invalid/wrong-input/wrong-key/wrong-program no-mutation, SBF/live lineage, reviewer identity/scope, findings disposition, and acceptance refs, but it is not audit/reviewer acceptance by itself, not SBF/live lineage, not tag-3 proof acceptance, and not C01 closure.

The C01 verifier evidence closure gate, `ops/mainnet/private-pool-v2-c01-verifier-evidence-closure-gate.evidence.json`, records `blocked-no-complete-c01-verifier-evidence-chain` and is checked by `npm run zk:c01-verifier-evidence-closure-gate-check`. A complete reviewed refs-only chain can be validated with `VANTA_C01_PRODUCTION_ARTIFACT_BUNDLE_PATH=<reviewed-bundle-json> VANTA_C01_VERIFIER_ADAPTER_ACCEPTANCE_PATH=<reviewed-adapter-json> VANTA_C01_SBF_LIVE_LINEAGE_ACCEPTANCE_PATH=<reviewed-lineage-json> VANTA_C01_AUDIT_REVIEWER_ACCEPTANCE_PATH=<reviewed-audit-json> npm run zk:c01-verifier-evidence-closure-gate-check`. This is composite evidence-chain intake only: it cross-checks reviewed production artifact bundle, verifier-adapter acceptance, SBF/live lineage acceptance, and audit/reviewer acceptance refs for one selected backend, proof tuple, reviewed frozen source commit, source-review acceptance and deterministic build/output-manifest prerequisite refs, reviewed beta18 H6 production source ACIR, reference current source ACIR hash, current H6 public input and adapter public-input binding flag, VK hash, verifier program upgrade-authority status ref, adapter refs, trim-normalized distinct mutation/no-mutation refs, lineage ref, audit ref, and required reviewer/producer attestations. Env-supplied C01 reviewed packets are rejected if they embed raw proof/VK/witness/key/secret/signed-transaction material instead of refs. It is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 external reviewer handoff packet, `ops/mainnet/private-pool-v2-c01-external-review-handoff.evidence.json`, records `operator-skipped-external-artifact-producer` and is checked by `npm run zk:c01-external-review-handoff-check`. It is the outbound refs-only package for source-review acceptance, production output-manifest preflight, deterministic production artifact build, production artifact bundle, verifier-adapter acceptance, SBF/live lineage acceptance, audit/reviewer acceptance, and composite evidence-chain closure. It maps each template, gate, env var, validation command, reviewed frozen source commit requirement, current H6 public input binding, reviewed beta18 H6 production source ACIR, reference current source ACIR hash, verifier program upgrade-authority status ref, and comparison-only local evidence boundary for reviewers. It is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The C01 production verifier artifact request packet, `ops/mainnet/private-pool-v2-c01-production-verifier-artifact-request.evidence.json`, records `ready-for-external-production-verifier-artifact-request-blocked` and is checked by `npm run zk:c01-production-verifier-artifact-request-check`. It turns that handoff into an artifact producer request by naming the reviewed frozen source commit requirement, source-freeze review acceptance, current and candidate source refs, current H6 public-input tuple, required production proof-format/VK/public-witness outputs, verifier-adapter acceptance, five trim-normalized distinct valid-mutation / invalid-proof / wrong-public-input / wrong-verifying-key / wrong-verifier-program evidence refs, deployed verifier program id/hash, verifier program upgrade-authority status ref, SBF/live lineage, audit/reviewer acceptance, and the validation commands for each returned refs-only packet. It is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not trim-normalized distinct mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

The local fail-closed verifier adapter seam harness is checked by `npm run zk:c01-verifier-adapter-seam-check`. It proves only that source has a separate tag-3 preflight, default adapter rejection, verified-commit helper, dedicated read-only executable verifier-program account precheck, generated Solana verifier CPI instruction shape with no account metas and data equal to `gnarkProof || gnarkPublicWitness`, an on-chain-only verifier CPI hook that passes the verifier-program account while host-side Solana syscall stubs remain fail-closed, commit-capable public tag-3 account list after test-only adapter acceptance, and that the default public tag-3 path rejects before account mutation. It is not verifier-adapter acceptance, not tag-3 proof acceptance, not production proof-format evidence, and not production verifying-key evidence.
...
Adding the Groth16 verifier is now a new proof-carrying spend ABI/verifier slice, not an extra field on the existing 161-byte transaction builder. The current program has useful scaffolding for `public_input_hash`, accepted roots, authority, nullifier markers, and output records, but the verifier work still needs production proof/VK artifacts, verifying-key commitment, and in-program or verifier-CPI acceptance before any payload can be treated as proof-enforced. The guarded C01 decision packet is `docs/zk/c01-production-verifier-backend-decision.md`; it records `groth16-tag3-solana-v0` as the selected production verifier backend direction and keeps the Noir/bb.js/UltraHonk adaptation path not selected. The C01 verifier-backend contract is intentionally explicit about the remaining boundary: current source tag `3` now reserves `verifierKeyHash:32`, `gnarkProof:324`, and `gnarkPublicWitness:44`, while rejecting the legacy 256-byte proof-only shape before account inspection. The remaining backend mismatch is local bb.js/UltraHonk proof evidence versus the selected Groth16-compatible Solana verifier path. Local bb.js/UltraHonk artifacts are not on-chain verifier evidence and remain `noir-bb` / `barretenberg-ultrahonk` local evidence (`local bb.js/UltraHonk artifacts are not on-chain verifier evidence`). Local `local-acir-bytecode-hash-not-production-vk` metadata is not on-chain verifier evidence; a positive lane needs `production-verifying-key-hash` evidence. The remote proof-artifact boundary now labels current receipts as `offchain-remote-proof-artifact-only` and rejects `solana-c01-groth16-verifier-ready` claims until Solana tag `3` Groth16 verifier-ready evidence exists for the actual-private-spend circuit, `private-spend-public-input-hash`, the selected Gnark-native proof/public-witness tuple, an accepted tag-3 adapter, and production verifying-key hash. Guards: `npm run zk:c01-verifier-backend-contract-check`; `npm run zk:c01-production-verifier-backend-candidate-check`; `npm run zk:c01-verifier-backend-decision-check`.

**Codex status, 2026-05-13 C01 verifier candidate evidence packet:** locally added a refs-only blocked evidence packet at `ops/mainnet/private-pool-v2-c01-verifier-candidate.evidence.json`. The packet now selects `groth16-tag3-solana-v0` as the C01 backend direction but does not make C01 verifier-ready; it records the required positive artifacts for actual-private-spend production proof format, `private-spend-public-input-hash` binding, `production-verifying-key-hash`, verifier adapter, valid-proof mutation test, invalid-proof no-mutation test, SBF/live lineage, and audit/reviewer acceptance. It preserves `offchain-remote-proof-artifact-only`, `local-acir-bytecode-hash-not-production-vk`, `selectedBackend: "groth16-tag3-solana-v0"`, `selectedBackendStatus: "selected-pending-production-evidence"`, `solanaC01Groth16VerifierReady: false`, and `privacyClaimAllowed: false`. Guards: `npm run zk:c01-production-verifier-backend-candidate-check`; `npm run public:audit-discovery-check`; `npm run audit:package-check`. This is backend-selection direction only, not production proof format evidence, not production verifying-key evidence, not on-chain verifier execution, not live deployment evidence, not audit acceptance, and not real-funds readiness.

**Codex status, 2026-05-13 C01 local proof-format observation:** locally added `ops/mainnet/private-pool-v2-c01-local-proof-format.evidence.json` and `npm run zk:c01-local-proof-format-evidence-check`. The guard self-generates the actual-private-spend local proof with `npm run private-pool-v2:actual-private-spend-prove`, verifies the generated receipt shape, and records that the current `vanta_private_pool_v2_actual_private_spend_entry` proof is `noir-bb` / `barretenberg-ultrahonk`, `local-bb-fixture-artifact`, 16000 bytes / 500 fields, one `private-spend-public-input-hash`, `local-acir-bytecode-hash-not-production-vk`, and `offchain-remote-proof-artifact-only`. It also records the mismatch with the reserved Solana tag `3` target, which expects `groth16`, 324 proof bytes, 44 public-witness bytes, 368 verifier instruction-data bytes, and `production-verifying-key-hash` evidence for `solana-c01-tag3-groth16-v0`. The C01 candidate packet now references this as intermediate evidence while reflecting `selectedBackend: "groth16-tag3-solana-v0"`, keeping all readiness booleans false, keeping all non-selection required positive evidence entries blocked, and keeping their `currentArtifactRef` fields null. Guard wiring: `npm run zk:c01-local-proof-format-evidence-check` is included in `npm run zk:review-guards-check`, `npm run zk:feedback-loop-check`, and therefore `npm run private-pool-v2:verify`. This is local proof-format observation only; it is not production proof-format acceptance, not production verifying-key evidence, not verifier-adapter acceptance, not tag-3 proof acceptance, not on-chain proof verification, not SBF/live redeploy or reinit evidence, not audit acceptance, not production-private readiness, and not real-funds readiness. Implementation commit: `28e07f6`.

**Codex status, 2026-05-13 C01 Verifier-Key Registry Scaffold, updated 2026-05-21 for verifier-program binding:** locally added source-only `TAG_REGISTER_VERIFIER_KEY = 5`, `ops/mainnet/private-pool-v2-c01-verifier-key-registry.evidence.json`, and `npm run zk:c01-verifier-key-registry-check` in implementation commit `a2fa2f4`, then extended the source scaffold so tag `5` is `[5, verifierKeyHash:32, verifierProgramId:32]`. It rejects zero verifier-key hashes, zero verifier program ids, and the spend program id as verifier program id, requires the initialized operator authority, verifies the `["vanta2vkey", pool_state, verifierKeyHash]` PDA, and creates or idempotently verifies a program-owned `VNTA2VKY` record containing the pool, verifier-key hash, and bound verifier program id. Reserved tag `3` now rejects a different executable verifier program before `ERR_PROOF_VERIFIER_NOT_WIRED`, but tag `3` still returns `ERR_PROOF_VERIFIER_NOT_WIRED` before proof verification, account creation, nullifier/output mutation, or spend acceptance. The C01 candidate packet references this registry scaffold as intermediate evidence while keeping `production-verifying-key-hash` blocked with `currentArtifactRef: null`. Guards: `cargo test --manifest-path programs/vanta_private_pool_v2_spend/Cargo.toml`; `npm run zk:c01-verifier-key-registry-check`; `npm run zk:c01-production-verifier-backend-candidate-check`. This is a source-only verifier-key registry scaffold, not backend selection, not production verifying-key evidence, not verifier-adapter acceptance, not tag-3 proof acceptance, not on-chain proof verification, not SBF/live redeploy or reinit evidence, not audit acceptance, not production-private readiness, and not real-funds readiness.
...
**Codex status, 2026-05-13 C01 Backend Options Matrix:** locally added `ops/mainnet/private-pool-v2-c01-verifier-backend-options.evidence.json` and `npm run zk:c01-verifier-backend-options-check` in implementation commit `1153f4e`. The packet now selects `groth16-tag3-solana-v0` as the backend direction, preserving the current tag `3` Groth16-shaped contract and requiring production proof-format, production verifying-key hash, verifier adapter/CPI, valid-proof mutation, invalid-proof no-mutation, live SBF lineage, and audit/reviewer evidence; it keeps `noir-bb-ultrahonk-adaptation` not selected because that path would require a new verifier target or production service boundary, production VK evidence that is not local ACIR metadata, a proof byte/service contract, public-input binding, acceptance/rejection evidence, live lineage, and audit/reviewer evidence. The C01 candidate packet now references this as selected direction evidence while keeping all non-selection positive evidence `currentArtifactRef` values null. Guards: `npm run zk:c01-verifier-backend-options-check`; `npm run zk:c01-production-verifier-backend-candidate-check`; `npm run zk:c01-verifier-backend-decision-check`. This matrix selects a backend direction only; it does not satisfy production proof-format evidence, production verifying-key evidence, verifier-adapter acceptance, tag-3 proof acceptance, on-chain verification, live SBF redeploy/reinit evidence, audit acceptance, production-private readiness, or real-funds readiness.

**Codex status, 2026-05-13 C01 Groth16 Proof-Format Candidate:** locally added `ops/mainnet/private-pool-v2-c01-groth16-proof-format-candidate.evidence.json` and `npm run zk:c01-groth16-proof-format-candidate-check` in implementation commit `faead7b`. The packet records `blocked-no-groth16-production-proof-format-artifact` for the selected `groth16-tag3-solana-v0` direction; the current required candidate shape is actual-private-spend, tag `3`, target `solana-c01-tag3-groth16-v0`, `proofSystem: "groth16"`, `proofFormatId: "gnark-solana-native-proof-and-public-witness-v0"`, 324 proof bytes, 44 public-witness bytes, 368 verifier instruction-data bytes, `private-spend-public-input-hash`, and `production-verifying-key-hash`. The current artifact is absent, the current spend-program tag-3 ABI now reserves the selected tuple fail-closed and rejects the legacy 256-byte proof-only shape, and the current local observation remains `noir-bb` / `barretenberg-ultrahonk` / 16000 bytes / 500 fields / `local-acir-bytecode-hash-not-production-vk`. The C01 candidate and backend-options packets now reference this as blocked selected-backend evidence while keeping all readiness booleans false and all non-selection positive evidence unsatisfied. Guard wiring: `npm run zk:c01-groth16-proof-format-candidate-check` is included in `npm run zk:review-guards-check`, `npm run zk:feedback-loop-check`, and `npm run private-pool-v2:verify`. This is selected-backend proof-format candidate evidence only; it is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not tag-3 proof acceptance, not on-chain proof verification, not SBF/live redeploy or reinit evidence, not audit acceptance, not production-private readiness, and not real-funds readiness.

**Codex status, 2026-05-13 C01 Production Verifying-Key Candidate:** locally added `ops/mainnet/private-pool-v2-c01-production-verifying-key-candidate.evidence.json` and `npm run zk:c01-production-verifying-key-candidate-check` in implementation commit `c82b23d`. The packet records `blocked-no-production-verifying-key-hash-artifact` for the selected `groth16-tag3-solana-v0` direction; the required shape is actual-private-spend, tag `3`, target `solana-c01-tag3-groth16-v0`, `proofSystem: "groth16"`, `proofFormatId: "gnark-solana-native-proof-and-public-witness-v0"`, 324 proof bytes, 44 public-witness bytes, 368 verifier instruction-data bytes, `private-spend-public-input-hash`, and `verifyingKeyHashKind: "production-verifying-key-hash"`. The current production verifying-key artifact is absent, tag `5` registry metadata remains source-only at `["vanta2vkey", pool_state, verifierKeyHash]`, and the current local observation remains `local-acir-bytecode-hash-not-production-vk`. The guard is schema-closed for the packet, rejects proof/VK/witness byte-bearing fields, and keeps `production-verifying-key-hash` positive evidence at `currentArtifactRef: null`; the C01 candidate, backend-options, Groth16 proof-format, audit package, and operator runbook now reference it as blocked selected-backend evidence. This is selected-backend verifying-key candidate evidence only; it is not production verifying-key evidence, not production proof-format evidence, not verifier-adapter acceptance, not tag-3 proof acceptance, not on-chain proof verification, not SBF/live redeploy or reinit evidence, not audit acceptance, not production-private readiness, and not real-funds readiness.

**Codex status, 2026-05-13 C01 Verifier Adapter Acceptance-Test Candidate:** locally added `ops/mainnet/private-pool-v2-c01-verifier-adapter-test-candidate.evidence.json` and `npm run zk:c01-verifier-adapter-test-candidate-check` in implementation commit `c0831fe`. The packet records `blocked-no-verifier-adapter-acceptance-tests` for the selected `groth16-tag3-solana-v0` direction; the required shape is actual-private-spend, tag `3`, target `solana-c01-tag3-groth16-v0`, `proofSystem: "groth16"`, `proofFormatId: "gnark-solana-native-proof-and-public-witness-v0"`, 324 proof bytes, 44 public-witness bytes, 368 verifier instruction-data bytes, one `private-spend-public-input-hash`, `production-verifying-key-hash`, and an in-program verifier or dedicated verifier CPI adapter. Current adapter artifact refs are null, and the packet records absent evidence for private-spend public-input hash binding, valid-proof mutation, invalid-proof no-mutation, wrong-public-input-hash no-mutation, wrong-verifying-key no-mutation, and wrong-verifier-program no-mutation. The C01 candidate and backend-options packets reference it as blocked selected-backend evidence while keeping all readiness booleans false and all non-selection required positive evidence refs null. Guard wiring: `npm run zk:c01-verifier-adapter-test-candidate-check` is included in `npm run zk:review-guards-check` and `npm run zk:feedback-loop-check`, and the active finding ledger now pins this as `VANTA-ZK-FEEDBACK-2026-05-13-C01-VERIFIER-ADAPTER-TEST-CANDIDATE`. This is selected-backend adapter candidate evidence only; it is not verifier-adapter acceptance, not proof acceptance, not production proof-format evidence, not production verifying-key evidence, not tag-3 proof acceptance, not on-chain proof verification, not SBF/live redeploy or reinit evidence, not audit acceptance, not production-private readiness, and not real-funds readiness.

**Codex status, 2026-05-20 local fail-closed verifier adapter seam harness:** locally added a source-only split between tag-3 preflight, default adapter rejection, and a verified-commit helper. `npm run zk:c01-verifier-adapter-seam-check` guards the default adapter rejecting before commit, the public fail-closed tag-3 path leaving preflighted accounts unchanged, the 368-byte `gnarkProof || gnarkPublicWitness` verifier instruction-data helper, the source public-witness binding precheck against `publicInputHash`, a commit-capable public tag-3 account list after test-only accepted-adapter success, the verified-commit helper mutating only after an explicit accepted-adapter handoff, and a test-only selected-Gnark fixture adapter shape covering valid mutation plus invalid-proof, wrong-public-input, wrong-verifying-key, and wrong-verifier-program no-mutation cases for the 324-byte proof plus 44-byte public-witness tuple. This moves C01 one step closer to a real verifier adapter interface, but it remains local drift-prevention only. It is not verifier-adapter acceptance, not tag-3 proof acceptance, not valid-proof mutation evidence for a production verifier, not invalid/wrong-public-input/wrong-verifying-key/wrong-verifier-program no-mutation evidence for a production verifier, not production proof-format evidence, not production verifying-key evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.

**Codex status, 2026-05-20 Sunspot/Nargo compatibility blocker:** locally tightened the selected Sunspot/Gnark route evidence so it records `blocked-local-nargo-version-mismatch-and-sunspot-missing`. Sunspot's upstream README requires Noir/Nargo `1.0.0-beta.18`, while this workspace reports `nargo 1.0.0-beta.19`; `sunspot` is not installed and `GNARK_VERIFIER_BIN` is not configured. A follow-up local dev probe records `local-dev-probe-succeeded-nonproduction-unsafe-setup-and-beta18-source-shim`: a temporary beta18 source shim traversed Sunspot compile/setup/prove/verify, built a local Solana verifier SBF artifact, and passed the generated standalone Solana verifier in local LiteSVM with the selected `gnark-solana-native-proof-and-public-witness-v0` tuple, a 324-byte proof plus 44-byte public witness. The current beta19 ACIR still panics Sunspot compile, Sunspot setup is unsafe/unaudited, the spend-program tag-3 ABI now reserves that selected tuple fail-closed and rejects the legacy 256-byte proof-only shape but still lacks an accepted adapter, Sunspot compile output reported zero public and zero secret inputs while the generated verifier records one public input, and no production proof/VK/Vanta-adapter/mutation/no-mutation/live/audit evidence is accepted. The route remains useful as the selected ACIR-to-Solana-Groth16 artifact direction, but it cannot produce C01 production proof/VK evidence until a compatible pinned toolchain, reviewed setup/toxic-waste mitigation, artifact pinning, verifier adapter evidence, SBF/live lineage, and reviewer/audit acceptance exist. Guards: `npm run zk:c01-production-groth16-toolchain-preflight-check`; `npm run zk:c01-sunspot-groth16-route-check`; `npm run zk:c01-sunspot-groth16-dev-probe-check`; `npm run zk:c01-sunspot-gnark-artifact-acquisition-check`.

**Codex status, 2026-05-20 C01 local public-witness binding observation:** locally added `ops/mainnet/private-pool-v2-c01-public-witness-binding.evidence.json` and `npm run zk:c01-public-witness-binding-check`. The packet now records that the nonproduction Sunspot/Gnark beta18 44-byte public witness decodes as a 12-byte Gnark header plus one 32-byte public input labeled `private-spend-public-input-hash`, but that value is stale against the current H6 actual-private-spend Noir/bb receipt. The source tag-3 default adapter still checks that same one-field public witness shape against `publicInputHash` before the not-wired verifier boundary. This narrows the public-witness decoding and source binding question, but it is still not production public-input binding evidence, not production proof-format evidence, not verifier-adapter acceptance, not valid-proof mutation evidence, not invalid/wrong-input/wrong-key/wrong-program no-mutation evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.

**Codex status, 2026-05-23 C01 production artifact H6 binding gate, updated 2026-05-24 UTC:** locally tightened `ops/mainnet/private-pool-v2-c01-production-artifact-acceptance-gate.evidence.json`, `ops/mainnet/private-pool-v2-c01-sunspot-gnark-artifact-acquisition.packet.json`, and `ops/mainnet/private-pool-v2-c01-sunspot-gnark-local-artifact-inventory.evidence.json` so returned production public-witness evidence cannot promote by byte lengths alone. The acceptance gate now preserves the current beta19 ACIR hash `sha256:a55defde42c5afba61a9cd7e96f350a407a88417312ce811a7c9bb97279b74f9` as the reference current source ACIR hash, but requires production output-manifest preflight, returned artifacts to bind the reviewed beta18 H6 production source ACIR `sha256:9c84b109bb2cf658e645bc971855ef06a8590c8b5b65398c6ae52afc431f8bde`, `productionSourceLineageMode: reviewed-beta18-h6-source-migration`, and one decoded `private-spend-public-input-hash` equal to the current H6 actual-private-spend proof receipt public input and commitment. The local artifact inventory now defaults to the H6-preserving beta18 output tuple and records proof `sha256:afde2c07683c4262b5b9ae72c66e559e857a9fd7a529b980b34a45bf2374d186`, public witness `sha256:19e42b6e34a5861d8804565476d72f8835c81d30cfc66bd598a236d26e1baa60`, VK `sha256:5e0a6f08503f534cbb462f43fbf0b247e8aa2ce75d1fa58c2350f815817948b4`, and verifier SBF `sha256:91fc2db5e06ebfb72bee120ebbcd51698684216598ec50f046d62fcf64928ac0`; it is still local unsafe comparison evidence only. The gate also has an executable refs-only reviewed-bundle intake template at `ops/mainnet/private-pool-v2-c01-production-artifact-bundle.template.json`, validated by `VANTA_C01_PRODUCTION_ARTIFACT_BUNDLE_PATH=<reviewed-refs-only-json> npm run zk:c01-production-artifact-acceptance-gate-check`. Guards: `npm run zk:c01-production-artifact-acceptance-gate-check`; `npm run zk:c01-production-output-manifest-check`; `npm run zk:c01-sunspot-gnark-artifact-acquisition-check`; `npm run zk:c01-sunspot-gnark-local-artifact-inventory-check`. This is production artifact intake hardening only; it is not production proof-format evidence, not production public-input binding evidence, not production verifying-key evidence, not verifier-adapter acceptance, not trim-normalized distinct mutation/no-mutation evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.

**Codex status, 2026-05-23/2026-05-24 UTC C01 current-source Sunspot compile blocker:** locally added `ops/mainnet/private-pool-v2-c01-current-source-sunspot-compile-attempt.evidence.json` and `npm run zk:c01-current-source-sunspot-compile-attempt-check`, wired into the C01 route/preflight/acquisition aggregates. The packet pins the current required source ACIR hash `sha256:a55defde42c5afba61a9cd7e96f350a407a88417312ce811a7c9bb97279b74f9` and records that its decompressed beta19 bytecode is version-prefixed MessagePack-like data, while the Sunspot beta18-era reader expects an old binary function-count layout and panics before CCS/proof/VK generation. Optional local-reader validation passed with `VANTA_C01_SUNSPOT_BIN=/private/tmp/vanta-c01-sunspot-lane/bin/sunspot VANTA_C01_SUNSPOT_COMPILE_LIVE=1 npm run zk:c01-current-source-sunspot-compile-attempt-check`. A fresh isolated build of latest observed upstream Sunspot main commit `e29fd6586f9a9f936ace0d71c103f5a4e9d9db76` also failed the current source ACIR with the same `go/acir/program.go:27` `makeslice: len out of range` path, and is reproducible with `VANTA_C01_SUNSPOT_BIN=/private/tmp/vanta-c01-sunspot-latest-e29fd658/go/sunspot VANTA_C01_SUNSPOT_COMPILE_ATTEMPT=latest VANTA_C01_SUNSPOT_COMPILE_LIVE=1 npm run zk:c01-current-source-sunspot-compile-attempt-check`. This is current-source blocker evidence only; C01 still needs a reviewed compatible toolchain or reviewed source migration, production setup/toxic-waste mitigation, deterministic production proof/VK/public-witness artifacts, accepted adapter, trim-normalized distinct mutation/no-mutation evidence, SBF/live lineage, and audit/reviewer acceptance.

**Codex status, 2026-05-23/2026-05-24 UTC C01 beta18 H6 migration probe packet:** locally added `ops/mainnet/private-pool-v2-c01-beta18-h6-migration-probe.evidence.json` and `npm run zk:c01-beta18-h6-migration-probe-check`, wired into the C01 source-migration and aggregate ZK guards. The packet records `local-h6-beta18-migration-probe-succeeded-nonproduction-unsafe-setup`: a separate local beta18-compatible source copy preserves the current H6 context preimage fields and Poseidon6 `computed_context_hash == context_hash` assertion, compiles/executes with Nargo beta18, passes Sunspot compile/setup/prove/verify, and produces a 44-byte Gnark public witness whose decoded `private-spend-public-input-hash` public witness matches the current H6 proof receipt. It also built a generated verifier SBF with hash `sha256:91fc2db5e06ebfb72bee120ebbcd51698684216598ec50f046d62fcf64928ac0`, passed the generated standalone LiteSVM verifier, and passed the Vanta local unsafe generated-verifier CPI harness through `npm run private-pool-v2:c01-local-unsafe-h6-verifier-cpi-acceptance-check`. This is the strongest local route evidence so far, but it is still not reviewed source migration, not production source lineage, not production setup/toxic-waste mitigation, not production proof-format evidence, not production VK/hash evidence, not production verifier-adapter acceptance, not trim-normalized distinct production mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 beta18 H6 source-migration review packet:** locally added `zk/noir/vanta_private_pool_v2_actual_private_spend_entry_sunspot_beta18_h6_candidate/`, `ops/mainnet/private-pool-v2-c01-beta18-h6-source-migration-review.evidence.json`, and `npm run zk:c01-beta18-h6-source-migration-review-check`. The in-repo candidate source hash is `sha256:caaeb2c2767965bd5d6c68c3043b8be10b43b345f017e32c6aa67658e927d430`, matching the H6 local probe source. After the 2026-06-04 C01 import compatibility fix, the current beta19 source has the same source hash and already uses `use dep::poseidon::poseidon::bn254;`; the compatibility marker is `no-source-delta-current-uses-beta18-compatible-poseidon-import`. The Prover fixture hash matches current source (`sha256:2c62fa9b0a7bfb32c98fb89fc5fbc90211d67b017066eb5555da8f0da20d890e`) and all H6 fields/assertions remain present. This turns the `/private/tmp` H6 source migration into a repo-local reviewer handoff, but it is still not reviewed source migration, not production source lineage, not production setup/toxic-waste mitigation, not deterministic reviewed production artifact build, not production proof-format evidence, not production VK/hash evidence, not production verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-23/2026-05-24 UTC C01 verifier-adapter acceptance gate:** locally added `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance-gate.evidence.json`, `ops/mainnet/private-pool-v2-c01-verifier-adapter-acceptance.template.json`, and `npm run zk:c01-verifier-adapter-acceptance-gate-check`, wired into the C01 production bundle and aggregate ZK guards. The gate records `blocked-no-production-verifier-adapter-acceptance` and validates refs-only reviewed adapter acceptance through `VANTA_C01_VERIFIER_ADAPTER_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-verifier-adapter-acceptance-gate-check`. It requires deterministic artifact build refs, production proof/VK/public-witness binding, accepted adapter/program boundary, valid mutation, and invalid/wrong-public-input/wrong-verifying-key/wrong-verifier-program no-mutation refs before adapter evidence can promote; those five refs must be distinct. This is adapter-acceptance intake only; it is not tag-3 production proof acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 audit/reviewer acceptance gate:** locally added `ops/mainnet/private-pool-v2-c01-audit-reviewer-acceptance-gate.evidence.json`, `ops/mainnet/private-pool-v2-c01-audit-reviewer-acceptance.template.json`, and `npm run zk:c01-audit-reviewer-acceptance-gate-check`, wired into the aggregate ZK guards. The gate records `operator-skipped-control` and validates refs-only final reviewer acceptance through `VANTA_C01_AUDIT_REVIEWER_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-audit-reviewer-acceptance-gate-check`. It requires reviewed production artifact bundle, production verifier-adapter acceptance, valid mutation, invalid/wrong-public-input/wrong-verifying-key/wrong-verifier-program no-mutation, SBF/live lineage, reviewer identity/scope, findings disposition, and acceptance refs before audit/reviewer acceptance can promote. This is final reviewer-intake only; it is not audit/reviewer acceptance by itself, not SBF/live lineage, not tag-3 proof acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 verifier evidence closure gate:** locally added `ops/mainnet/private-pool-v2-c01-verifier-evidence-closure-gate.evidence.json` and `npm run zk:c01-verifier-evidence-closure-gate-check`, wired into the aggregate ZK guards. The gate records `blocked-no-complete-c01-verifier-evidence-chain` and validates a complete reviewed refs-only evidence chain through `VANTA_C01_PRODUCTION_ARTIFACT_BUNDLE_PATH=<reviewed-bundle-json> VANTA_C01_VERIFIER_ADAPTER_ACCEPTANCE_PATH=<reviewed-adapter-json> VANTA_C01_SBF_LIVE_LINEAGE_ACCEPTANCE_PATH=<reviewed-lineage-json> VANTA_C01_AUDIT_REVIEWER_ACCEPTANCE_PATH=<reviewed-audit-json> npm run zk:c01-verifier-evidence-closure-gate-check`. It cross-checks the reviewed production artifact bundle, production verifier-adapter acceptance, SBF/live lineage acceptance, and audit/reviewer acceptance packets against the same selected backend, route, frozen source commit, source-review acceptance and deterministic build/output-manifest prerequisite refs, reviewed beta18 H6 production source ACIR, reference current source ACIR hash, current H6 public input binding and adapter public-input binding flag, production verifying-key hash, verifier program upgrade-authority status ref, adapter refs, trim-normalized distinct mutation/no-mutation refs, lineage ref, and audit ref. The C01 env-var intake now rejects reviewed JSON that embeds raw proof/VK/witness/key/secret/signed-transaction material instead of refs. This is composite closure intake only; it is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 external reviewer handoff packet:** locally added `ops/mainnet/private-pool-v2-c01-external-review-handoff.evidence.json` and `npm run zk:c01-external-review-handoff-check`, wired into the aggregate ZK guards. The packet records `operator-skipped-external-artifact-producer` and gives reviewers one refs-only map for source-review acceptance, production output-manifest preflight, deterministic production artifact build, production artifact bundle, verifier-adapter acceptance, SBF/live lineage acceptance, audit/reviewer acceptance, and composite evidence-chain closure, including the frozen source commit and verifier program upgrade-authority status ref. It points to every template/gate/env-var command and preserves the current H6 public-input binding plus comparison-only local evidence boundaries. This is reviewer-handoff intake only; it is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 production verifier artifact request packet:** locally added `ops/mainnet/private-pool-v2-c01-production-verifier-artifact-request.evidence.json` and `npm run zk:c01-production-verifier-artifact-request-check`, wired into the aggregate ZK guards. The packet records `ready-for-external-production-verifier-artifact-request-blocked` and packages the artifact producer request for frozen source commit/source tree status refs, source-review acceptance, deterministic production artifact build, production proof-format/VK/public-witness refs, verifier-adapter acceptance, five trim-normalized distinct valid-mutation / invalid-proof / wrong-public-input / wrong-verifying-key / wrong-verifier-program evidence refs, deployed verifier program id/hash, verifier program upgrade-authority status ref, SBF/live lineage, audit/reviewer acceptance, and composite closure. It cross-links the artifact acquisition packet, external handoff packet, current/candidate source refs, current H6 public-input tuple, validation commands, and comparison-only local evidence. This is request metadata only; it is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not trim-normalized distinct mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-24 UTC C01 production output-manifest preflight:** locally added `ops/mainnet/private-pool-v2-c01-production-output-manifest-preflight.evidence.json` and `npm run zk:c01-production-output-manifest-check`, wired into the aggregate ZK guards. The command validates its blocked refs-only packet, can hash and shape-check a returned artifact directory with `VANTA_C01_PRODUCTION_OUTPUT_MANIFEST_ROOT=<returned-artifact-dir> npm run zk:c01-production-output-manifest-check`, and can revalidate a refs-only output manifest with `VANTA_C01_PRODUCTION_OUTPUT_MANIFEST_PATH=<refs-only-output-manifest-json> npm run zk:c01-production-output-manifest-check`. This is output-manifest preflight only; it is not production proof-format evidence, not production verifying-key evidence, not deterministic build acceptance, not verifier-adapter acceptance, not trim-normalized distinct mutation/no-mutation evidence, not SBF/live lineage, not audit/reviewer acceptance, and not C01 closure.

**Codex status, 2026-05-21 C01 verifier CPI hook hardening, updated 2026-06-04 UTC:** locally extended the source-only tag-3 seam so preflight now requires a dedicated read-only executable verifier-program account after the verifier-key PDA, the default adapter constructs the generated Solana verifier CPI instruction shape with no account metas and data equal to `gnarkProof || gnarkPublicWitness`, and SBF builds have an on-chain-only verifier CPI hook that passes the verifier-program account while host-side Solana syscall stubs remain fail-closed. `npm run zk:c01-verifier-adapter-seam-check` now guards the instruction shape, verifier-program preflight, public-witness binding, and host fail-closed behavior. `npm run private-pool-v2:c01-sbf-verifier-cpi-rejection-check`, now included in `npm run private-pool-v2:crucible-check`, proves the spend SBF can reach an executable verifier-program CPI and leave accounts unchanged when that verifier rejects. `npm run private-pool-v2:c01-local-unsafe-verifier-cpi-acceptance-check` now points at a repo-pinned H6 local unsafe Sunspot verifier SBF plus `/private/tmp` reference proof/public-witness metadata; it proves the spend SBF can CPI into that verifier, accept the H6 proof/public-witness tuple, mutate nullifier/output state after verifier success, reject a tampered proof without mutation, reject a wrong public input hash without mutation, and reject a wrong executable verifier program without mutation when the verifier-key record is bound to the correct generated verifier program id. The optional pre-H6 wrong-verifying-key negative-control leg remains nonproduction and is skipped in current runs unless the transient pre-H6 verifier SBF is present. This is verifier-adapter interface and local unsafe SBF CPI hardening only. It is not production verifier adapter acceptance, not tag-3 production proof acceptance, not valid-proof mutation evidence for a production verifier, not production wrong-verifying-key or wrong-verifier-program no-mutation evidence, not production proof-format evidence, not production verifying-key evidence, not SBF/live lineage, not audit acceptance, and not C01 closure.

**Codex status, 2026-05-23 C01 SBF/live lineage candidate packet, updated 2026-06-04 UTC:** locally added `ops/mainnet/private-pool-v2-c01-sbf-live-lineage-candidate.evidence.json` and `npm run zk:c01-sbf-live-lineage-candidate-check`. The packet records `blocked-no-rebuilt-redeployed-reinitialized-live-lineage` for the selected `groth16-tag3-solana-v0` path and defines the refs reviewers need before live lineage can promote: rebuilt spend SBF hash, accepted verifier SBF hash or reviewed in-program verifier hash, deployed spend/verifier program ids, verifier program upgrade-authority status ref, tag-5 production verifier-key registration, reinitialization or migration receipts, live proof-enforced tag-3 receipt or reviewer-accepted dry-run receipt, and audit/reviewer acceptance. It now also records `local-h6-sbf-lineage-rehearsal-only`, a refs-only local dry run binding the local spend SBF hash `sha256:a30be245fa0b00ea703f077b2f67463132556a3ee20a6efe9364297f7b08e9ad`, H6 generated verifier SBF hash `sha256:91fc2db5e06ebfb72bee120ebbcd51698684216598ec50f046d62fcf64928ac0`, local unsafe H6 VK hash `sha256:5e0a6f08503f534cbb462f43fbf0b247e8aa2ce75d1fa58c2350f815817948b4`, and local mutation/no-mutation matrix. The SBF/live lineage acceptance gate `ops/mainnet/private-pool-v2-c01-sbf-live-lineage-acceptance-gate.evidence.json` now records `blocked-no-sbf-live-lineage-acceptance` and validates refs-only returned lineage packets through `VANTA_C01_SBF_LIVE_LINEAGE_ACCEPTANCE_PATH=<reviewed-refs-only-json> npm run zk:c01-sbf-live-lineage-acceptance-gate-check`. Local SBF ABI freshness, the local H6 rehearsal, and `/private/tmp` generated verifier SBF hashes remain comparison-only and not SBF/live lineage. This is a blocked lineage intake surface only; it is not production proof-format evidence, not production verifying-key evidence, not verifier-adapter acceptance, not trim-normalized distinct production mutation/no-mutation evidence, not tag-3 proof acceptance, not audit acceptance, and not C01 closure.
...
**Codex status, 2026-05-12 reserved proof-carrying/root-provenance ABI:** partially remediated locally for fail-closed verifier-boundary shape and source-only root provenance. `programs/vanta_private_pool_v2_spend/src/lib.rs` reserves tag `3` with the current selected Gnark tuple payload that carries the existing spend transcript plus `verifierKeyHash`, `gnarkProof:324`, and `gnarkPublicWitness:44`, rejects malformed/all-zero public transcript, proof, or public-witness placeholders, and rejects the legacy 449-byte / 256-byte proof-only shape before account inspection. It preflights pool/nullifier-set/output-index/root-history/root-record/nullifier-marker/output-record accounts plus a read-only verifier-key PDA derived from `["vanta2vkey", pool_state, verifierKeyHash]` and a bound executable verifier-program account. Host builds return custom error `14` before mutation; SBF builds can reach verifier CPI and now have local rejection/no-mutation coverage, but no accepted production verifier exists. The same source reserves `TAG_REGISTER_PROVENANCED_ROOT = 4`, which creates a lineage-bound program-owned root provenance record from `["vanta2root", pool_state, acceptedRoot]`; reserved tag `3` and tag `6` require that record before failing closed, while legacy tag `2` roots remain outside that provenance lane unless migrated/reinitialized. The SBF ABI status surface reports `proofCarryingSpendStatus: "sbf-verifier-cpi-hook-host-fail-closed-production-blocked"`, `verifierKeyAccountLen: 112`, `registerVerifierKeyInstructionLen: 65`, and `spendWithProofAccountCount: 11`, while the current transaction builder still requires tag `1` and 161-byte spend instruction data. `npm run zk:c01-verifier-backend-contract-check` keeps the Groth16-compatible verifier target, local bb.js/UltraHonk fixture boundary, and production verifying-key hash requirement distinct. This is not a verifier, not proof acceptance, not proof that the root transition is correct, not a program-owned shared tree, not deployed/live SBF program evidence, not audit acceptance, and not Target A.
