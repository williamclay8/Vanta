...
5. **Local unsafe Sunspot/Gnark acceptance harness.** Same fuzz file, `spend_with_proof_local_unsafe_generated_verifier_cpi_acceptance_and_no_mutation`. Calls the generated verifier SBF from `/private/tmp/vanta-c01-sunspot-lane/work/beta18-circuit/target/`, demonstrates valid→mutation, invalid→no-mutation, wrong-public-input→no-mutation, wrong-verifier-program→no-mutation. **`provesWrongVerifyingKeyNoMutation: false`** in the evidence packet — the wrong-VK leg is still open even at the local-unsafe level.
...
| Production wrong-verifying-key no-mutation | **blocked** | The local unsafe harness explicitly records `provesWrongVerifyingKeyNoMutation: false`. There is no wrong-VK leg that runs against an executable verifier program bound to the *correct* program id but the *wrong* VK hash. This needs an adapter-level test that swaps VK bytes (or VK hash registration) and demonstrates the verifier CPI rejects and accounts stay byte-identical. |
...
1. **Add a wrong-verifying-key no-mutation leg to the local unsafe Sunspot harness.** The current `spend_with_proof_local_unsafe_generated_verifier_cpi_acceptance_and_no_mutation` test proves four of the five no-mutation cases but explicitly leaves the wrong-VK case open. The packet field is already shaped (`provesWrongVerifyingKeyNoMutation`, `satisfiesWrongVerifyingKeyNoMutation`). Implementing this would *not* satisfy the production gate, but it would close a known local-coverage hole. Concrete plan: register a second `verifier_key` PDA bound to the same correct verifier program id but with a different `verifierKeyHash`, run the same valid (proof, public-witness) tuple through it, and assert the CPI rejects (or the registry binding rejects before CPI) with zero byte-level mutation. Mirror in the evidence packet by flipping `provesWrongVerifyingKeyNoMutation: false → true` but keep all `satisfies*` flags at `false`.
2. **Pin the Nargo beta-18 source-shim story in `docs/zk/c01-production-verifier-backend-decision.md`.** The packet already records that the Sunspot dev probe required a beta-18-only import shim and that current beta19 ACIR panics Sunspot compile. The decision doc mentions this but does not currently track a one-line plan for resolving the toolchain mismatch (pin Nargo, port Sunspot to beta19, or wait for Sunspot upstream). Without that decision committed, the production-toolchain-preflight gate stays stuck even if all the other gates close. This is a 10-minute doc edit.
3. **Promote the AUDIT_2026-05-19 H1/H2 cleanup decision to the goal tracker.** That audit flagged the dead `src/zk/indexerClient.ts` / `src/zk/clientProver.ts` path, the dead `vanta-light-public-indexer.onrender.com` deploy, and three misleading "Production sign-off" docs. None of these block C01, but they are exactly the kind of out-of-tracker drift that confuses reviewers when they read the C01 boundary. Either delete the dead code + docs, or move them under a `STATUS=draft` label. Keeping them as-is muddies the C01 honesty story even though the C01 evidence packets themselves are clean.
