...
- Production restore-drill evidence: `ops/mainnet/production-restore-drill.evidence.json` records that the Private Pool v2 operator restore database was created by reference only; readback remains pending until `npm run mainnet:production-restore-drill-readback` is run with the restored DB URL in a local shell or secret-manager context.
