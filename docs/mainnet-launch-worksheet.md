...
- Production restore-drill evidence: `ops/mainnet/production-restore-drill.evidence.json` records that the operator/control-plane restore database passed readback for `VANTA_OPERATOR_DATABASE_URL_REF`; the separate Private Pool v2 core restore gate for `VANTA_PRIVATE_POOL_V2_DATABASE_URL_REF` remains pending.
