...
The `payGrowthLoopDiscovery` section makes the Pay receipt growth loop publicly discoverable for reviewers and counterparties. It points to `vanta-pay-growth-loop-discovery-0.1`, the `vanta-pay-growth-loop-evidence-v0.1` fixture-evidence schema, the `vanta-pay-live-growth-loop-measurement-v0.1` live redacted first-party measurement schema, the counterparty verifier route `/receipt/:receiptId`, and the guard `npm run pay:growth-loop-check`. The companion `payMeasuredLoopImplementation` packet points to `vanta-pay-measured-loop-implementation-v0.1` and `npm run pay:measured-loop-implementation-check`, proving that runtime event capture, redacted operator intake, status discovery, public discovery, and claim controls are wired together. The `payCounterpartyActivation` packet points to `vanta-pay-counterparty-activation-v0.1` and `npm run pay:counterparty-activation-check`; it turns the verified receipt into a policy-safe next action, `Request private settlement`, and measures `counterparty_invite_created`, `counterparty_invite_opened`, and `next_settlement_intent_created` as redacted intent events. The `payCommittedCheckoutAcceptance` packet points to `vanta-pay-committed-checkout-acceptance-v0.1` and `npm run pay:committed-checkout-acceptance-check`; it makes the receipt-bound next action inspectable as `Accept committed checkout`, verifies the committed-economics checkout boundary, and measures `committed_checkout_acceptance_created` as redacted metadata. The live event intake stores only redacted event metadata through `POST /v1/growth-loop/events` and can be read through `GET /v1/growth-loop/status`; no live adoption, legal/compliance approval, production privacy, anonymity, audit acceptance, signing, broadcast, real-funds, or mainnet readiness claim is lifted.
...
The local SBF verifier-CPI rejection harness is guarded by `npm run private-pool-v2:c01-sbf-verifier-cpi-rejection-check` and included in `npm run private-pool-v2:crucible-check`. It proves the spend SBF can reach an executable verifier-program CPI and leave accounts unchanged when that verifier rejects. This is rejection/no-mutation drift prevention only; it is not accepted verifier evidence, not valid production mutation, not production invalid/wrong-input/wrong-key/wrong-program no-mutation evidence, not SBF/live lineage, and not audit acceptance.

The local unsafe generated-verifier CPI harness is guarded by `npm run private-pool-v2:c01-local-unsafe-verifier-cpi-acceptance-check`. It loads the H6-preserving Sunspot/Gnark generated verifier SBF, proof, and public witness into the Crucible/TestContext lane and proves the Vanta spend SBF can CPI into that verifier, accept the local unsafe H6 proof/public-witness tuple, mutate nullifier/output state after verifier success, reject a tampered proof without mutation, reject a wrong public input hash without mutation, and reject a wrong executable verifier program without mutation when the verifier-key record is bound to the correct generated verifier program id. The optional pre-H6 wrong-verifying-key negative-control leg is skipped unless its transient verifier SBF is present. This is local unsafe Sunspot/Gnark evidence only; it is not production verifier-adapter acceptance, not production proof-format evidence, not production verifying-key evidence, not production wrong-verifying-key or wrong-verifier-program no-mutation evidence, not SBF/live lineage, and not audit acceptance.

The fail-closed Groth16 verifier-CPI production guard is `npm run private-pool-v2:groth16-verifier-cpi-check`. It pins the tag `3` Groth16 CPI ABI markers, verifier-key-to-verifier-program binding, no-accounts verifier instruction shape, host-side `ERR_PROOF_VERIFIER_NOT_WIRED` behavior, and the external C01 artifact/adapter/SBF-live/audit blockers. It is a drift guard only; it is not production verifier evidence, not accepted mutation/no-mutation evidence, not deployed verifier program lineage, not tag-5 verifier-key registration evidence, and not audit acceptance.
