...
- fixture events: `receipt_generated`, `share_link_copied`, `counterparty_verifier_opened`, `next_private_settlement_requested`, `counterparty_invite_created`, `counterparty_invite_opened`, `next_settlement_intent_created`, and `committed_checkout_acceptance_created`
...
The receipt growth loop is a beta trust-packet, verifier-surface, fixture-measurement primitive, claim-blocked live redacted measurement surface, measured loop implementation packet, counterparty activation packet, and committed checkout acceptance packet. The implementation packet ties together runtime event capture, redacted operator intake, status discovery, public discovery, and claim controls. The counterparty activation packet makes the receipt actionable by naming the next policy-safe action, `Request private settlement`, and the redacted intent events that can be measured. The committed checkout acceptance packet makes the next action reviewable as `Accept committed checkout`, binds it to hidden-economics and committed-economics checkout acceptance guards, and measures only redacted `committed_checkout_acceptance_created` metadata. It is not a claim of adoption, live institutional volume, production privacy, signing, broadcast, real-funds movement, or legal/compliance approval.
