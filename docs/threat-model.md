...
- Hybrid ZK + selective disclosure boundary: ZK proves validity, not every product policy boundary. Vanta must keep local/client-side proving as the default privacy direction, but private-settlement products also need non-ZK mechanisms such as scoped viewing keys, revocation lists, blinded-token relayer access, policy allowlists, and operator runbooks where those mechanisms are the actual enforcement layer. Selective disclosure must be explicit, time/scope-limited, and receipt-bound; it must not become broad de-anonymization or an implicit compliance claim.
