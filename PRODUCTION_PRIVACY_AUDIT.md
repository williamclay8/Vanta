...
Current local source update (2026-05-25 / PPA-DISCOVERY-002): local legacy v1 Send memo migration tooling can re-encrypt parse-compatible v1 Send history into sanitized v2 recipient/change discovery metadata when recipient and change viewing-key material is present, or produce segregation-required records for missing keys, malformed payloads, fresh v2 memos, and non-Send memos. It does not rewrite historical on-chain plaintext memo content, deploy recipient discovery, or lift Send production privacy claims. Reviewed migration/segregation evidence is still required.
...
| High | **Memo plaintext leakage risk on legacy v1.** Live Send writes v2 AEAD (`SECURITY_LIMITATIONS.md:55`) but legacy v1 memos are still readable. Anyone with historic vault state can read them. | client + ops | Local source now has migration/segregation tooling guarded by `npm run actions:legacy-v1-send-memo-migration-check`; it still requires a scheduled reviewed migration/segregation job before production Send privacy claims can cover legacy history. |
