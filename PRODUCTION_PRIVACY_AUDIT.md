...
| Critical | **Recipient discovery is local-only.** Today `private-pool-v2-service-network.mjs` writes "send discovery packets" to its indexer JSON snapshot. Anyone with the operator's filesystem can correlate sender → recipient by storage timestamp ordering. The encrypted memo body is opaque, but the *fact* that recipient X received a packet is observable. | ops + protocol | Adopt view-tags (recipient publishes a tag = `H(viewing_key, ephemeral_pub)` and the indexer is queried by tag prefix). Recipients pull all packets matching their tag prefix without revealing which exact packet they own. Pattern is in `vantaShieldViewingKey.ts`; needs an indexer-side endpoint. |
