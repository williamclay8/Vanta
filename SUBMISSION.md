...
- `npm run private-core:shipping-status`
  prints the compact operator-backed shipping summary for the frozen narrow zk-v1 lane, including the summary-state version, mirrored contract version, current summary generation time, and the supporting finish-line, required-lanes, release-boundary, contract-mirror, and boundary summaries
