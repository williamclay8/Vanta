...
assert(proofArtifact.circuit === "vanta_private_pool_v2_shield_entry", "artifact must name Shield circuit.");
...
assert(
  JSON.stringify(proofArtifact.publicInputLabels) === JSON.stringify(["shield-public-input-hash"]),
  "artifact must label the Shield public input hash.",
);
...
assertVantaPrivatePoolV2ProofArtifactHasNoWitnessMaterial(
  proofArtifact,
  "Private Pool v2 Shield proof artifact",
);

const { verifyVantaPrivatePoolV2ShieldProofArtifact } = await import(
  "../operator/private-pool-v2-proof-artifact.mjs"
);
assert(
  typeof verifyVantaPrivatePoolV2ShieldProofArtifact === "function",
  "Shield proof artifact verifier export is required.",
);

async function expectReject(label, artifact, expectedMessageFragment) {
  try {
    await verifyVantaPrivatePoolV2ShieldProofArtifact({ proofArtifact: artifact });
  } catch (error) {
...
const verifiedReceipt = await verifyVantaPrivatePoolV2ShieldProofArtifact({ proofArtifact });
assert(verifiedReceipt.verified === true, "valid artifact must verify.");
assert(
  verifiedReceipt.verifiedPublicInputs.shieldPublicInputHash === proofArtifact.publicInputs[0],
  "verified receipt must decode the Shield public-input hash.",
);
console.log("private-pool-v2 Shield proof artifact no-witness verification: PASS");

await expectReject(
  "private-pool-v2 Shield proof artifact tampered proof rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact tampered public input rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact circuit relabel rejection",
  {
    ...proofArtifact,
    circuit: "vanta_private_pool_v2_send_entry",
  },
  "vanta_private_pool_v2_shield_entry circuit",
);

await expectReject(
  "private-pool-v2 Shield proof artifact proofSystem relabel rejection",
  {
    ...proofArtifact,
    proofSystem: "mock",
  },
...
await expectReject(
  "private-pool-v2 Shield proof artifact backend relabel rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact public input label relabel rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact commitment mismatch rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact proofBackend mismatch rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact malformed proofHex rejection",
  {
    ...proofArtifact,
    proofHex: "not-hex",
  },
...
await expectReject(
  "private-pool-v2 Shield proof artifact extra public input rejection",
  {
    ...proofArtifact,
    publicInputCommitment: createVantaPrivatePoolV2ProofArtifactPublicInputCommitment([
...
await expectReject(
  "private-pool-v2 Shield proof artifact verifyingKeyHash tamper rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact acirBytecodeHash tamper rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact verifyingKeyId tamper rejection",
  {
    ...proofArtifact,
    verifyingKeyId: "local-acir-bytecode:vanta_private_pool_v2_shield_entry:sha256:00",
  },
  "verifyingKeyId mismatch",
);

await expectReject(
  "private-pool-v2 Shield proof artifact witness sidecar rejection",
  {
    ...proofArtifact,
    witnessSource: "target/vanta_private_pool_v2_shield_entry.gz",
  },
  "forbidden no-witness field",
);

await expectReject(
  "private-pool-v2 Shield proof artifact witness alias rejection",
  {
    ...proofArtifact,
    witness: { amount: "5000" },
  },
...
await expectReject(
  "private-pool-v2 Shield proof artifact privateInputs alias rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact private_inputs alias rejection",
  {
    ...proofArtifact,
...
await expectReject(
  "private-pool-v2 Shield proof artifact noteSecret alias rejection",
  {
    ...proofArtifact,
...
console.log("Vanta Private Pool v2 Shield proof artifact consistency check: PASS");
