...
for (const [field, value] of Object.entries(packet.positiveClaimRequires)) {
  assert(value === false, `positive claim requirement ${field} must remain false`);
}
...
assert(
  packet.positiveEvidenceRefs.candidatePacketStatus === "blocked-no-production-verifier-backend-selected",
...
assert(candidate.status === "blocked-no-production-verifier-backend-selected", "candidate must remain blocked");
...
for (const id of ["accepted-proof-mutates-state-test", "invalid-proof-leaves-accounts-unchanged-test"]) {
