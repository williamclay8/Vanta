...
const tempRoot = mkdtempSync(resolve(repoRoot, ".tmp/vanta-private-pool-v2-swap-to-shielded-no-witness-"));
...
const port = 13080 + Math.floor(Math.random() * 300);
...
const proofArtifact = JSON.parse(readFileSync(swapToShieldedArtifactPath, "utf8"));
...
const operator = startOperator(port, "private-pool-v2-swap-to-shielded-no-witness.json");
...
try {
  ...
  assert(
    accepted.parsed?.verifiedReceipt?.verifiedPublicInputs?.swapPublicInputHash ===
      proofArtifact.publicInputs[0],
    "Expected verified receipt to decode Swap-to-shielded public input hash.",
  );
  assert(
    accepted.parsed?.kind === "Private Pool V2 Swap-to-shielded proof artifact verification",
    "Expected operator to name the Swap-to-shielded proof artifact route.",
  );
  console.log("private-pool-v2 operator Swap-to-shielded proof artifact no-witness acceptance: PASS");

  await expectReject(
    "private-pool-v2 operator Swap-to-shielded missing expected public input rejection",
    { proofArtifact },
    "expectedPublicInputs.swapPublicInputHash",
  );
  await expectReject(
    "private-pool-v2 operator Swap-to-shielded mismatched expected public input rejection",
    {
      expectedPublicInputs: {
        swapPublicInputHash: (BigInt(proofArtifact.publicInputs[0]) + 1n).toString(10),
      },
      proofArtifact,
    },
  ...
  await expectReject(
    "private-pool-v2 operator Swap-to-shielded cannot satisfy Claim expected input",
    {
  ...
  await expectReject(
    "private-pool-v2 operator Swap-to-shielded cannot satisfy actual-private expected input",
    {
  ...
  await expectReject(
    "private-pool-v2 operator Swap-to-shielded mixed witness alias rejection",
    { expectedPublicInputs, proofArtifact, witness: { nullifier: "123" } },
    "strict no-witness",
  );
  await expectReject(
    "private-pool-v2 operator Swap-to-shielded nested privateInputs rejection",
    {
      expectedPublicInputs,
  ...
  await expectReject(
    "private-pool-v2 operator Send artifact cannot satisfy Swap-to-shielded expected input",
    { expectedPublicInputs, proofArtifact: sendProofArtifact },
    "swapPublicInputHash",
  );
  await expectReject(
    "private-pool-v2 operator Claim artifact cannot satisfy Swap-to-shielded expected input",
    { expectedPublicInputs, proofArtifact: claimProofArtifact },
    "swapPublicInputHash",
  );
  await expectReject(
    "private-pool-v2 operator actual-private artifact cannot satisfy Swap-to-shielded expected input",
    { expectedPublicInputs, proofArtifact: actualPrivateSpendProofArtifact },
    "swapPublicInputHash",
  );
  await expectReject(
    "private-pool-v2 operator relabelled Send artifact rejected for Swap-to-shielded",
    {
      expectedPublicInputs,
      proofArtifact: {
        ...sendProofArtifact,
...
const productionOperator = startOperator(
  port + 1_000,
  "private-pool-v2-swap-to-shielded-production.json",
  {
...
console.log("Vanta Private Pool v2 Swap-to-shielded operator no-witness check: PASS");
