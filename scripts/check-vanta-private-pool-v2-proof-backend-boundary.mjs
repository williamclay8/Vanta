...
const claimBrowserWorkerProverCheck = read(
  "scripts/check-vanta-private-pool-v2-claim-browser-worker-prover.mjs",
);
const swapToShieldedBrowserWorkerProverCheck = read(
...
includes(
  localProver,
  "derived Shield, Claim, Swap-to-shielded, actual-private-spend, and Send artifacts are generated by local dev-only proof paths",
...
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
  "proveVantaPrivatePoolV2ActualPrivateSpendInBrowserWorker",
  "actual-private-spend browser worker check exported prover",
);
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
  "createVantaPrivatePoolV2ActualPrivateSpendCircuitFixture",
  "actual-private-spend browser worker check fixture source",
);
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
  "private-spend-public-input-hash",
  "actual-private-spend browser worker check public input label",
);
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
  "assertVantaPrivatePoolV2ProofArtifactHasNoWitnessMaterial",
  "actual-private-spend browser worker check no-witness artifact guard",
);
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
  "actual-private-spend structured clone boom",
  "actual-private-spend browser worker check postMessage cleanup guard",
);
includes(
  actualPrivateSpendBrowserWorkerProverCheck,
...
includes(
  claimBrowserWorkerProverCheck,
  "proveVantaPrivatePoolV2ClaimInBrowserWorker",
  "Claim browser worker check exported prover",
);
includes(
  claimBrowserWorkerProverCheck,
  "createVantaPrivatePoolV2ClaimCircuitFixture",
  "Claim browser worker check fixture source",
);
includes(
  claimBrowserWorkerProverCheck,
  "claim-public-input-hash",
  "Claim browser worker check public input label",
);
includes(
  claimBrowserWorkerProverCheck,
  "assertVantaPrivatePoolV2ProofArtifactHasNoWitnessMaterial",
  "Claim browser worker check no-witness artifact guard",
);
includes(
  claimBrowserWorkerProverCheck,
  "Claim structured clone boom",
  "Claim browser worker check postMessage cleanup guard",
);
includes(
  claimBrowserWorkerProverCheck,
  "rejected the Claim witness input",
  "Claim browser worker check sanitized witness-input error",
);
includes(
  swapToShieldedBrowserWorkerProverCheck,
  "vanta-private-pool-v2-browser-worker-prove-swap-to-shielded",
  "Swap-to-shielded browser worker check request kind",
);
includes(
  swapToShieldedBrowserWorkerProverCheck,
  "Private Pool v2 Swap-to-shielded browser worker witness-input proof artifact",
  "Swap-to-shielded browser worker check witness-input artifact",
);
includes(
  swapToShieldedBrowserWorkerProverCheck,
  "Swap structured clone boom",
  "Swap-to-shielded browser worker check postMessage cleanup guard",
);
includes(
  swapToShieldedBrowserWorkerProverCheck,
  "rejected the Swap-to-shielded witness input",
  "Swap-to-shielded browser worker check sanitized witness-input error",
);
...
assert(
  scripts["private-pool-v2:claim-browser-worker-prover-check"] ===
...
assert(
  scripts["private-pool-v2:swap-to-shielded-browser-worker-prover-check"] ===
...
includes(
  browserWorkerProofResultAdapter,
  "proveClaim",
  "browser-worker proof-result adapter Claim delegation",
);
includes(
  browserWorkerProofResultAdapter,
  "claim-public-input-hash",
  "browser-worker proof-result adapter Claim public input label",
);
includes(
  browserWorkerProofResultAdapter,
  "proveSwapToShielded",
  "browser-worker proof-result adapter Swap-to-shielded delegation",
);
includes(
  browserWorkerProofResultAdapter,
  "swap-public-input-hash",
  "browser-worker proof-result adapter Swap-to-shielded public input label",
);
includes(
  browserWorkerProofResultAdapter,
...
assert(
  ...
  "package.json private-pool-v2:local-prover-check must include private-pool-v2:claim-browser-worker-prover-check",
);
assert(
  scripts["private-pool-v2:local-prover-check"]?.includes(
    "npm run private-pool-v2:swap-to-shielded-browser-worker-prover-check",
  ),
  "package.json private-pool-v2:local-prover-check must include private-pool-v2:swap-to-shielded-browser-worker-prover-check",
);
assert(
  scripts["private-pool-v2:local-prover-check"]?.includes(
...
assert(
  ...
  "private-pool-v2:verify must reach the Claim browser worker prover guard through proof-backend-boundary-check",
);
assert(
  scripts["private-pool-v2:verify"]?.includes("npm run private-pool-v2:proof-backend-boundary-check") &&
    scripts["private-pool-v2:proof-backend-boundary-check"]?.includes(
  ...
  "private-pool-v2:verify must reach the Swap-to-shielded browser worker prover guard through proof-backend-boundary-check",
);
assert(
  scripts["private-pool-v2:verify"]?.includes("npm run private-pool-v2:proof-backend-boundary-check") &&
    scripts["private-pool-v2:proof-backend-boundary-check"]?.includes(
