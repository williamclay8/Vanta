...
const proofBackendGateIndex = swapPage.search(
  /proofReceipt\.proofBackend\s*!==\s*"local-bb-derived-artifact"/u,
);
const proofSystemGateIndex = swapPage.search(/proofReceipt\.proofSystem\s*!==\s*"noir-bb"/u);
const persistProofIndex = swapPage.indexOf(
  "persistCanonicalSwapBrowserLocalProofReceipt",
  proofSystemGateIndex,
);

assert(
  browserProofCallIndex >= 0 &&
    proofBackendGateIndex > browserProofCallIndex &&
    proofSystemGateIndex > proofBackendGateIndex &&
    persistProofIndex > proofSystemGateIndex,
  "Swap page must fail closed on browser-local proof receipt backend/system before persistence.",
);
