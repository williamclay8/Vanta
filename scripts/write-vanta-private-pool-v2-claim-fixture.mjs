...
const outputPath = resolve(
  repoRoot,
  "zk/noir/vanta_private_pool_v2_claim_entry/Prover.toml",
);
...
if (
  ...
  console.error(
    'Expected fixture mode "valid", "invalid-binding", or "invalid-nullifier".',
  );
  process.exit(1);
}

const tempRoot = mkdtempSync(resolve(repoRoot, ".tmp/vanta-private-pool-v2-claim-fixture-"));
...
try {
  ...
  const fixtureModule = await import(
    pathToFileURL(join(tempJsDir, "privatePoolV2ClaimCircuitFixture.js")).href
  );
  const fixture = fixtureModule.createVantaPrivatePoolV2ClaimCircuitFixture({
    mode: fixtureMode,
  });
  const toml = fixtureModule.serializeVantaPrivatePoolV2ClaimCircuitFixtureToToml(fixture);
