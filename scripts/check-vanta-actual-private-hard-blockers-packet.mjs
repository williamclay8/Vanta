...
const expectedStopBoundary = {
  liveMainnetActionsAllowedNow: approval.liveMainnetActionsAllowedNow,
  currentApprovalActionRef: approval.approvalActionRef,
  currentApprovalWindowRef: approval.approvalWindowRef,
  stopConditionStatus: approval.stopCondition.appliesToCurrentApproval ? "fired" : "not-applicable-to-current-approval",
  fundsMoved: false,
};
const expectedFreshBoundedApprovalStatus = approval.liveMainnetActionsAllowedNow
  ? "satisfied-active-bounded-approval"
  : "blocked-expired-bounded-approval";
...
try {
  ...
  assert.equal(packet.stopBoundary.liveMainnetActionsAllowedNow, expectedStopBoundary.liveMainnetActionsAllowedNow);
  assert.equal(packet.stopBoundary.stopConditionStatus, expectedStopBoundary.stopConditionStatus);
  assert.equal(packet.stopBoundary.fundsMoved, expectedStopBoundary.fundsMoved);
  assert.equal(packet.stopBoundary.currentApprovalActionRef, expectedStopBoundary.currentApprovalActionRef);
  assert.equal(packet.stopBoundary.currentApprovalWindowRef, expectedStopBoundary.currentApprovalWindowRef);
  ...
  assert.equal(blockers.get("fresh-bounded-approval").status, expectedFreshBoundedApprovalStatus);
  assert.equal(
    blockers.get("fresh-bounded-approval").currentArtifactRef,
    `approval-window:${approval.approvalWindowRef.replace(" ", "-").replace("/", "-")}`,
  );
  assert.equal(
    blockers.get("shared-cohort-deposit-transaction").currentArtifactRef,
    "review:shared-cohort-deposit-ref-not-yet-solscan-final-reviewed-2055-2220",
  );
  assert.equal(
    blockers.get("shared-cohort-deposit-transaction").requiredArtifactShape,
    "solana-tx:<shared-cohort-deposit-mainnet-signature>",
  );
  assert.equal(
    blockers.get("authenticated-production-duplicate-replay-rejection").requiredArtifactShape,
  ...
  console.error("Vanta actual-private hard blockers packet check: FAIL");
  console.error(
    JSON.stringify(
      {
        packetPath: "ops/mainnet/actual-private-hard-blockers.packet.json",
        packetCheckedAt: packet.checkedAt ?? null,
        packetStopBoundary: packet.stopBoundary ?? null,
        expectedStopBoundary,
        expectedFreshBoundedApprovalStatus,
        approval: {
          checkedAt: approval.checkedAt ?? null,
          currentTimeZoneClock: approval.currentTimeZoneClock ?? null,
          approvalWindowStatus: approval.approvalWindowStatus ?? null,
          liveMainnetActionsAllowedNow: approval.liveMainnetActionsAllowedNow ?? null,
          stopCondition: approval.stopCondition ?? null,
        },
      },
      null,
      2,
