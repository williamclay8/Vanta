...
- The current Private Pool v2 prover is still local benchmark infrastructure, even though the repo also includes Noir circuit checks, local proof generation, and a local remote-proof-artifact handoff guard. That guard rejects relabelled local fixture metadata and binds returned remote verifier receipts to submitted artifact transcript fields, but it is still local handoff hardening and not production remote proving, deployed verifier evidence, or mainnet-private settlement.
