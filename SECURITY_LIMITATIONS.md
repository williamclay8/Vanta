...
- Pay checkout now routes operator-backed live checkout settlement through the Private Pool v2 committed `send` protocol endpoint with stateful private-send commitments. Pay withdrawal now routes the configured-operator path through committed `unshield` protocol settlement with commitment-only economics/route/exit handles instead of raw `/pay-settlements`. This is still not production payment privacy: the lane remains local benchmark infrastructure without audited anonymity-set evidence, relayer-separated production execution, live mainnet settlement, or production readiness.
...
- Strategy planning/runtime can derive a redacted private-rail handoff and committed-economics request packet for local verification, and the local Strategy operator runtime can queue those packets behind a fail-closed live-submission gate. It does not yet submit live Jupiter swaps, Jito bundles, or private-settlement transactions.
...
- Vanta has local Private Pool v2 committed-economics protocol settlement coverage for Send, Swap, and Unshield receipts.
- Vanta has local Private Pool v2 proof-request boundaries for Send, Swap, and Unshield that replace raw economics with commitments at that typed request layer.
