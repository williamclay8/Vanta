...
import {
  ...
  fetchVantaPrivateCoreOperatorReleasePackage,
  fetchVantaPrivateCoreOperatorReleasePackageCheck,
  ...
  type VantaPrivateCoreOperatorReleasePackageCheckResponse,
  type VantaPrivateCoreOperatorReleasePackageResponse,
  ...
  type VantaPrivateCoreOperatorShippingArtifactResponse,
...
function abbreviatePrivateCoreValue(value: string | null | undefined) {
  if (!value) {
    return null;
  }
...
export type PrivacyFlowContextValue = {
  privatePoolV2LatestProtocolSettlement: VantaProtocolSettlementResponse | null;
  ...
  privateCoreOperatorSupportedReleaseCandidateGateTransport: string | null;
  privateCoreOperatorSupportedReleaseCandidateGateEndpoint: string | null;
  privateCoreOperatorSupportedReleaseCandidateTransport: string | null;
  privateCoreOperatorSupportedReleaseCandidateEndpoint: string | null;
...
export type VantaPrivateCoreReleaseWorkflowState = {
  releaseCandidateId: string | null;
  prepareStatusLabel: string;
  preparePrimaryNote: string;
  checkStatusLabel: string;
  checkPrimaryNote: string;
  shipStatusLabel: string;
  shipPrimaryNote: string;
...
export type VantaPrivateCoreReleasePackageState = {
  releaseCandidateId: string | null;
  packageIdentityLabel: string;
  packageStatusLabel: string;
  packagePrimaryNote: string;
  gateStatusLabel: string;
  gatePrimaryNote: string;
  artifactIdentityLabel: string;
  decisionIdentityLabel: string;
...
export function PrivacyFlowProvider({ children }: { children: ReactNode }) {
  ...
  const [privateCoreOperatorSupportedShippingArtifactEndpoint, setPrivateCoreOperatorSupportedShippingArtifactEndpoint] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateVersion, setPrivateCoreOperatorSupportedReleaseCandidateVersion] =
    useState<number | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateKind, setPrivateCoreOperatorSupportedReleaseCandidateKind] =
    useState<string | null>(null);
  ...
  const [privateCoreOperatorSupportedReleaseCandidateGateKind, setPrivateCoreOperatorSupportedReleaseCandidateGateKind] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateGateNote, setPrivateCoreOperatorSupportedReleaseCandidateGateNote] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateGateTransport, setPrivateCoreOperatorSupportedReleaseCandidateGateTransport] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateGateEndpoint, setPrivateCoreOperatorSupportedReleaseCandidateGateEndpoint] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateTransport, setPrivateCoreOperatorSupportedReleaseCandidateTransport] =
    useState<string | null>(null);
  const [privateCoreOperatorSupportedReleaseCandidateEndpoint, setPrivateCoreOperatorSupportedReleaseCandidateEndpoint] =
    useState<string | null>(null);
  ...
  const [privateCoreOperatorReleasePackage, setPrivateCoreOperatorReleasePackage] =
    useState<VantaPrivateCoreOperatorReleasePackageResponse | null>(null);
  const [privateCoreOperatorReleasePackageCheck, setPrivateCoreOperatorReleasePackageCheck] =
    useState<VantaPrivateCoreOperatorReleasePackageCheckResponse | null>(null);
  const [privateCoreOperatorShippingArtifact, setPrivateCoreOperatorShippingArtifact] =
    useState<VantaPrivateCoreOperatorShippingArtifactResponse | null>(null);
  ...
  const [privatePoolV2LatestProtocolSettlement, setPrivatePoolV2LatestProtocolSettlement] =
    useState<VantaProtocolSettlementResponse | null>(null);
  ...
  const refreshPrivateCoreOperatorSummary = useCallback(async () => {
    ...
    applyPrivateCoreOperatorContractState({
      ...
      setPrivateCoreOperatorSupportedReleaseCandidateGateTransport,
      setPrivateCoreOperatorSupportedReleaseCandidateGateEndpoint,
      setPrivateCoreOperatorSupportedReleaseCandidateTransport,
      setPrivateCoreOperatorSupportedReleaseCandidateEndpoint,
    ...
    setPrivateCoreOperatorShippingArtifact(shippingArtifactState);
    ...
    setPrivateCoreOperatorReleasePackage(releasePackageState);
    setPrivateCoreOperatorReleasePackageCheck(releasePackageCheckState);
  ...
  const privateCoreReleaseWorkflowState = useMemo(
    () =>
      summarizePrivateCoreReleaseWorkflowState({
        candidate: privateCoreOperatorReleaseCandidate,
        candidateCheck: privateCoreOperatorReleaseCandidateCheck,
  ...
  const privateCoreReleasePackageState = useMemo(
    () =>
      summarizePrivateCoreReleasePackageState({
        releasePackage: privateCoreOperatorReleasePackage,
        releasePackageCheck: privateCoreOperatorReleasePackageCheck,
        handoff: privateCoreReleaseHandoffState,
      }),
    [
      privateCoreOperatorReleasePackage,
      privateCoreOperatorReleasePackageCheck,
  ...
  const runPrivateCoreShield = useCallback((args: {
    ...
    void recordPrivatePoolV2ProtocolSettlement({
      ...
      if (settlement) {
        setPrivatePoolV2LatestProtocolSettlement(settlement);
      }
      return refreshPrivatePoolV2ProtocolSettlementStatus();
    });
...
function summarizePrivateCoreReleaseWorkflowState(args: {
  candidate: VantaPrivateCoreOperatorReleaseCandidateResponse | null;
  candidateCheck: VantaPrivateCoreOperatorReleaseCandidateCheckResponse | null;
  shippingArtifact: VantaPrivateCoreOperatorShippingArtifactResponse | null;
}): VantaPrivateCoreReleaseWorkflowState | null {
  const candidate = args.candidateCheck?.candidate ?? args.candidate;
  const artifact = args.shippingArtifact;

  if (!candidate && !artifact) {
    return null;
  }

  const releaseCandidateId = candidate?.releaseCandidateId ?? artifact?.releaseCandidateId ?? null;
  const prepareStatusLabel = candidate?.sendId ? "Candidate prepared" : "Awaiting primary send";
  const preparePrimaryNote = candidate?.sendId
    ? "The canonical primary private send created an exact release-candidate lineage."
    : "Run the primary private send path to create the exact release candidate.";
  const checkStatusLabel =
    args.candidateCheck?.decisionStatus === "ready"
      ? "Candidate check ready"
      : args.candidateCheck?.decisionStatus === "blocked"
        ? "Candidate check blocked"
        : candidate
          ? "Candidate check pending"
          : "Awaiting candidate";
  const checkPrimaryNote =
    args.candidateCheck?.decisionNote ??
    candidate?.lineageNote ??
    "Exact release-candidate check is waiting for the canonical primary lane.";
  const shipStatusLabel =
    artifact?.decisionStatus === "ready-to-ship"
      ? "Shipping artifact ready"
      : artifact?.decisionStatus === "blocked"
        ? "Shipping artifact blocked"
        : "Awaiting shipping artifact";
  const shipPrimaryNote =
    artifact?.decisionNote ??
    "The exact release-candidate shipping artifact is waiting for the frozen narrow lane to become ship-ready.";
  const artifactStatusLabel = artifact
    ? `Artifact v${String(artifact.artifactVersion)} · ${artifact.artifactKind}`
    : "Awaiting artifact";
  const artifactPrimaryNote = artifact
    ? "The shipping artifact bundles the exact release-candidate lineage with the current operator snapshot."
    : "No exact release-candidate shipping artifact has been observed yet.";
  const releasedAmountLabel =
    artifact?.latestReleasedAmount && artifact.latestReleasedAssetId
      ? formatPrivateCoreAssetAmount(
          artifact.latestReleasedAssetId,
          BigInt(artifact.latestReleasedAmount),
        )
      : null;
  ...
  return {
    ...
    artifactStatusLabel,
    artifactPrimaryNote,
    releaseRequestId: artifact?.latestReleaseRequestId ?? candidate?.releaseRequestId ?? null,
    releasedAmount: releasedAmountLabel,
    noteSummary: releasedAmountLabel
...
function summarizePrivateCoreReleasePackageState(args: {
  releasePackage: VantaPrivateCoreOperatorReleasePackageResponse | null;
  releasePackageCheck: VantaPrivateCoreOperatorReleasePackageCheckResponse | null;
  ...
  const releasePackageCheck = args.releasePackageCheck;
  const handoff = args.handoff;

  if (!releasePackage && !handoff) {
    return null;
  }

  const releaseCandidateId = releasePackage?.releaseCandidateId ?? handoff?.releaseCandidateId ?? null;
  const packageIdentityLabel = releasePackage
  ...
  const packageStatusLabel =
    releasePackage?.packageStatus === "ready"
      ? "Release package ready"
      : releasePackage?.packageStatus === "blocked"
        ? "Release package blocked"
        : handoff?.packageStatusLabel ?? "Release package unavailable";
  const packagePrimaryNote =
    releasePackage?.packageNote ??
  ...
  const gateStatusLabel =
    releasePackageCheck?.decisionStatus === "ready"
      ? "Package gate ready"
      : releasePackageCheck?.decisionStatus === "blocked"
        ? "Package gate blocked"
        : packageStatusLabel;
  const gatePrimaryNote = releasePackageCheck?.decisionNote ?? packagePrimaryNote;
  const artifactIdentityLabel = releasePackage
    ? `Artifact v${String(releasePackage.artifactVersion)} · ${releasePackage.artifactKind}`
    : handoff?.artifactIdentityLabel ?? "Awaiting shipping artifact";
  const decisionIdentityLabel = releasePackage
    ? `Decision v${String(releasePackage.decisionVersion)} · ${releasePackage.decisionKind}`
    : handoff?.decisionIdentityLabel ?? "Awaiting shipping decision";
  const contractIdentityLabel = releasePackage
    ? `Contract v${String(releasePackage.contractVersion)} · Summary v${String(releasePackage.summaryVersion)}`
    : "Contract identity unavailable";
  const snapshotIdentityLabel = releasePackage
    ? `Snapshot v${String(releasePackage.snapshotVersion)} · ${releasePackage.snapshotKind}`
    : "Snapshot identity unavailable";
  const generatedAt = releasePackage?.summaryGenerated ?? null;
  const summaryGeneratedLabel = generatedAt
    ...
    : "Unavailable";
  const currentRootLabel = releasePackage?.currentRoot ?? "Unavailable";
  const latestProofLabel = releasePackage?.latestProofId ?? "Unavailable";
  const latestSendLabel = releasePackage?.latestSendId ?? "Unavailable";
  const latestReleaseLabel = releasePackage?.latestReleaseRequestId ?? "Unavailable";
  const lineageSummaryLabel = releasePackage
    ? [
        `Root ${abbreviatePrivateCoreValue(releasePackage.currentRoot) ?? "Unavailable"}`,
        `Send ${abbreviatePrivateCoreValue(releasePackage.latestSendId) ?? "Unavailable"}`,
  ...
  const exportPayload = {
    releaseCandidateId,
    packageIdentity: packageIdentityLabel,
    packageStatus: packageStatusLabel,
    packageNote: packagePrimaryNote,
    packageGateStatus: gateStatusLabel,
    packageGateNote: gatePrimaryNote,
    ...
    lineageSummary: lineageSummaryLabel,
    observationMode: "Operator-owned release package",
  };

  const exportText = [
    `Release package identity: ${packageIdentityLabel}`,
    `Release package: ${packageStatusLabel}`,
    `Package note: ${packagePrimaryNote}`,
    `Package gate: ${gateStatusLabel}`,
    `Package gate note: ${gatePrimaryNote}`,
    ...
    `Lineage summary: ${lineageSummaryLabel}`,
  ].join("\n");
...
function applyPrivateCoreOperatorContractState(args: {
  ...
  setPrivateCoreOperatorSupportedReleaseCandidateGateTransport: (value: string | null) => void;
  setPrivateCoreOperatorSupportedReleaseCandidateGateEndpoint: (value: string | null) => void;
  setPrivateCoreOperatorSupportedReleaseCandidateTransport: (value: string | null) => void;
  setPrivateCoreOperatorSupportedReleaseCandidateEndpoint: (value: string | null) => void;
  ...
  args.setPrivateCoreOperatorSupportedReleaseCandidateVersion(
    args.contractState.supportedReleaseCandidateVersion,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateKind(
    args.contractState.supportedReleaseCandidateKind,
  );
  ...
  args.setPrivateCoreOperatorSupportedReleaseCandidateGateKind(
    args.contractState.supportedReleaseCandidateGateKind,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateGateNote(
    args.contractState.supportedReleaseCandidateGateNote,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateGateTransport(
    args.contractState.supportedReleaseCandidateGateTransport,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateGateEndpoint(
    args.contractState.supportedReleaseCandidateGateEndpoint,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateTransport(
    args.contractState.supportedReleaseCandidateTransport,
  );
  args.setPrivateCoreOperatorSupportedReleaseCandidateEndpoint(
    args.contractState.supportedReleaseCandidateEndpoint,
  );
