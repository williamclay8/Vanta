...
export function getSendTrustContract(): SendTrustContract {
  return {
    ...
    visibleStatusCopy:
      "Fresh v2 Send memos put ciphertext, signer, and timing on chain. A local dual-AEAD scaffold can separately seal recipient and change discovery memos with ciphertext body hashes, and the Private Pool v2 Send proof-request/circuit lane locally binds those body-hash limbs into the public input hash. External Send remains fail-closed until recipient viewing-key exchange, view-tag/indexer discovery, live evidence, and audit gates are wired. Operator/status surfaces still see transition and proof metadata.",
